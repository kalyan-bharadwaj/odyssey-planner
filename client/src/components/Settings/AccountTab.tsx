...
const MFA_BACKUP_SESSION_KEY = 'odyssey_mfa_backup_codes_pending';
...
export default function AccountTab(): React.ReactElement {
  ...
  const downloadBackupCodes = () => {
    ...
    a.href = url;
    a.download = 'odyssey-mfa-backup-codes.txt';
  ...
  const printBackupCodes = () => {
    if (!backupCodesText) return;
    const html = `<!doctype html><html><head><meta charset="utf-8"/><title>ODYSSEY MFA Backup Codes</title>
      <style>body{font-family:Arial,sans-serif;padding:32px}h1{font-size:20px}pre{font-size:16px;line-height:1.6}</style>
      </head><body><h1>ODYSSEY MFA Backup Codes</h1><p>${new Date().toLocaleString()}</p><pre>${backupCodesText}</pre></body></html>`;
      ...
      <Section title={t('settings.account')} icon={User}>
        ...
        {!oidcOnlyMode && (
          <div className="mt-4 border-t border-edge-secondary pt-4">
            <label className="mb-3 block text-sm font-medium text-slate-700">{t('settings.changePassword')}</label>
        ...
        <div className="mt-4 border-t border-edge-secondary pt-4">
          <div className="mb-3 flex items-center gap-2">
            <KeyRound className="h-5 w-5 text-content-secondary" />
            <h3 className="m-0 text-base font-semibold text-content">{t('settings.mfa.title')}</h3>
          </div>
          <div className="space-y-3">
            {mfaRequiredByPolicy && (
              <div className="flex gap-3 rounded-lg border border-edge bg-surface-secondary p-3 text-sm text-content">
            ...
            {demoMode ? (
              <p className="m-0 text-sm text-amber-700">{t('settings.mfa.demoBlocked')}</p>
            ) : (
              <>
                ...
                {!user?.mfa_enabled && mfaQr && (
                  <div className="space-y-3">
                    ...
                    <div
                      className="mx-auto block overflow-hidden rounded-lg border border-edge"
                      style={{ width: 'fit-content' }}
                      dangerouslySetInnerHTML={{ __html: mfaQr! }}
                    />
                    <div>
                      <label className="mb-1 block text-xs font-medium text-content-secondary">
                        {t('settings.mfa.secretLabel')}
                      </label>
                      <code className="block break-all rounded bg-surface-hover p-2 text-xs text-content">
                        {mfaSecret}
                      </code>
                    </div>
                    <input
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        onClick={async () => {
                          setMfaLoading(true);
                          try {
                            const resp = (await authApi.mfaEnable({ code: mfaSetupCode })) as {
                              backup_codes?: string[];
                            };
                ...
                {backupCodes && backupCodes.length > 0 && (
                  <div className="space-y-3 rounded-lg border border-edge bg-surface-hover p-3">
                    <p className="m-0 text-sm font-semibold text-content">{t('settings.mfa.backupTitle')}</p>
                    <p className="m-0 text-xs text-content-muted">{t('settings.mfa.backupDescription')}</p>
                    <pre
                      className="m-0 overflow-auto rounded border border-edge bg-surface-card p-2 text-xs text-content"
                      style={{ maxHeight: 220 }}
                    >
                      {backupCodesText}
                    </pre>
                    <p className="m-0 text-xs text-[#b45309]">{t('settings.mfa.backupWarning')}</p>
                    <div className="flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={copyBackupCodes}
                        className="flex items-center gap-1.5 rounded-lg border border-edge px-3 py-2 text-xs text-content-secondary"
                      >
                      ...
                      <button
                        ...
                        className="rounded-lg border border-edge px-3 py-2 text-xs text-content-secondary"
                      >
                        {t('common.ok')}
                      </button>
                    </div>
          ...
          <div className="flex flex-col gap-1">
            <div className="text-sm text-content-muted">
              ...
              >
                {user?.role === 'admin' ? (
                  <>
                    <Shield size={13} /> {t('settings.roleAdmin')}
                  </>
                ) : (
                  t('settings.roleUser')
                )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12 }}>
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-lg border border-[#fecaca] px-4 py-2 text-sm font-medium text-red-500 transition-colors hover:bg-red-50"
          >
            <Trash2 size={14} />
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              ...
              <h3 className="text-content" style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>
