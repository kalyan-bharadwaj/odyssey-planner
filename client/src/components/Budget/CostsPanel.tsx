...
import { CURRENCIES, SPLIT_COLORS, SYMBOLS } from './BudgetPanel.constants';
...
type LedgerEntry = { kind: 'expense'; date: string; e: BudgetItem } | { kind: 'payment'; date: string; s: Settlement };
...
export default function CostsPanel({ tripId, tripMembers = [] }: CostsPanelProps) {
  ...
  const personName = useCallback(
    (id: number) => (id === me ? t('costs.you') : personById(id)?.username || '?'),
    [me, personById, t]
  );
  ...
  const initial = useCallback(
    (id: number) => (id === me ? t('costs.youShort') : (personById(id)?.username || '?').charAt(0).toUpperCase()),
    [me, personById, t]
  );
  ...
  const dateMeta = useMemo(() => {
    ...
    try {
      const s = new Date(trip.start_date + 'T00:00:00Z'),
        e = new Date(trip.end_date + 'T00:00:00Z');
  ...
  const bigMoney = (amount: number, smallSize: number, mutedColor: string) => {
    ...
    try {
      ...
      parts = new Intl.NumberFormat(currencyLocale(base), {
        style: 'currency',
        currency: base,
      ...
      return <>{formatMoney(amount, base, locale)}</>;
    }
    const isBig = (p: Intl.NumberFormatPart) => p.type === 'integer' || p.type === 'group' || p.type === 'minusSign';
    return (
      <>
        {parts.map((p, i) => (
          <span key={i} style={isBig(p) ? undefined : { fontSize: smallSize, fontWeight: 500, color: mutedColor }}>
            {p.value}
          </span>
        ))}
      </>
        ...
        <div style={{ maxWidth: '100%', margin: '0 auto' }}>
          ...
          >
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {dateMeta && (
                <span
                  className="border border-edge bg-surface-card text-content-muted"
                  style={{
                    display: 'inline-flex',
                    alignItems: 'center',
                    gap: 6,
          ...
          {/* ── Summary cards ── */}
          <div
            style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.15fr', gap: 16, marginBottom: 36 }}
            className="costs-summary"
          >
            <SummaryCard
              label={t('costs.youOwe')}
              sub={t('costs.youOweSub')}
              amount={totals.owe}
              currency={base}
              locale={locale}
              ...
              foot={
                totals.owe > 0.01 ? (
                  <FlowPills
                    ids={(settlement?.flows || []).filter((f) => f.from.user_id === me).map((f) => f.to.user_id)}
                    lead={t('costs.to')}
                    Avatar={Avatar}
                    name={personName}
                  />
            ...
            />
            <SummaryCard
              label={t('costs.youreOwed')}
              sub={t('costs.youreOwedSub')}
              amount={totals.owed}
              currency={base}
              locale={locale}
              ...
              foot={
                totals.owed > 0.01 ? (
                  <FlowPills
                    ids={(settlement?.flows || []).filter((f) => f.to.user_id === me).map((f) => f.from.user_id)}
                    lead={t('costs.from')}
                    Avatar={Avatar}
                    name={personName}
                  />
            ...
            />
            <SummaryCard
              label={t('costs.totalSpend')}
              sub={t('costs.totalSpendSub')}
              amount={totals.totalSpend}
              currency={base}
              locale={locale}
              ...
              foot={
                <span style={{ display: 'flex', gap: 16 }}>
                  <span>
                    {t('costs.yourShare')} · <b>{fmt0(totals.myShare)}</b>
                  </span>
                  <span>
                    {t('costs.youPaid')} · <b>{fmt0(totals.myPaid)}</b>
                  </span>
                </span>
              }
            />
          ...
          {/* ── Main grid ── */}
          <div
            style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 32, alignItems: 'start' }}
            ...
            <div>
              ...
              >
                <h3
                  ...
                  style={{ fontSize: 24, fontWeight: 600, letterSpacing: '-0.025em', margin: 0 }}
                >
                  {t('costs.expenses')}
                </h3>
                ...
                dayGroups.map((g) => {
                  ...
                  return (
                    <div key={g.day} style={{ marginBottom: 22 }}>
                      <div
                        className={labelCls}
                        style={{ display: 'flex', alignItems: 'center', margin: '0 0 10px 4px' }}
                      >
                        {g.day}
                        <span
              ...
              <div className={cardCls} style={{ borderRadius: 22, padding: '22px 24px' }}>
                ...
                >
                  <div className={labelCls}>
                    {t('costs.settleUp')} · <span className="text-content">{(settlement?.flows || []).length}</span>
                  </div>
                  {canEdit && (
                    <button
    ...
    if (flows.length === 0)
      return (
        <div style={{ textAlign: 'center', padding: '14px 8px' }}>
          ...
          <div className="text-content" style={{ fontSize: 14.5, fontWeight: 600 }}>
        ...
        {flows.map((f, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
            <div
              ...
              title={`${personName(f.from.user_id)} → ${f.to.user_id === me ? t('costs.youLower') : personName(f.to.user_id)}`}
            >
              <Avatar id={f.from.user_id} size={32} />
              <ArrowRight size={15} className="text-content-faint" />
              <Avatar id={f.to.user_id} size={32} />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
              <span className="text-content" style={{ fontSize: 14, fontWeight: 700 }}>
                {fmt(f.amount)}
              </span>
              {canEdit && (
                <button
                  onClick={() => settleFlow(f.from.user_id, f.to.user_id, f.amount)}
                  className="bg-[var(--text-primary)] text-[var(--bg-primary)]"
                  style={{
                    padding: '7px 12px',
                    borderRadius: 9,
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, paddingTop: 8 }}>
        ...
        >
          ...
          >
            <span>
              {t('costs.yourShare')} · <b style={{ color: '#fff', fontWeight: 600 }}>{fmt0(totals.myShare)}</b>
            </span>
            <span>
              {t('costs.youPaid')} · <b style={{ color: '#fff', fontWeight: 600 }}>{fmt0(totals.myPaid)}</b>
            </span>
          </div>
          {canEdit && (
            <button
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <div className={cardCls} style={{ borderRadius: 18, padding: 16 }}>
            ...
            <div className="text-content" style={{ fontSize: 12.5, fontWeight: 600 }}>
              {t('costs.youOwe')}
            </div>
            <div className="text-content-faint" style={{ fontSize: 10.5 }}>
              {t('costs.youOweSub')}
            </div>
            <div
              style={{
        ...
        <div className={cardCls} style={{ borderRadius: 18, padding: 16 }}>
          <div
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, gap: 8 }}
          >
            <div
              className="text-content"
              style={{
            ...
            >
              {t('costs.settleUp')}{' '}
              <span className="text-content-faint" style={{ fontSize: 12, fontWeight: 500 }}>
        ...
        {/* Expenses */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="text-content" style={{ fontSize: 19, fontWeight: 700, letterSpacing: '-0.02em' }}>
            {t('costs.expenses')}
          </div>
          <div
            className="border border-edge bg-surface-card"
            style={{ display: 'flex', alignItems: 'center', gap: 8, borderRadius: 12, padding: '0 12px', height: 42 }}
          >
          ...
          {dayGroups.length === 0 ? (
            <div className="text-content-faint" style={{ textAlign: 'center', padding: '36px 16px', fontSize: 13 }}>
              {search ? t('costs.noMatch') : t('costs.emptyText')}
            </div>
          ) : (
            dayGroups.map((g) => {
              ...
              return (
                <div key={g.day} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div className={labelCls} style={{ display: 'flex', alignItems: 'center', padding: '0 2px' }}>
                    {g.day}
                    <span
                      className="text-content-muted"
                      style={{
  ...
  function ExpenseRow({ e }: { e: BudgetItem }) {
    ...
    return (
      <div
        className="exp-row border border-edge bg-surface-card"
        style={{
          display: 'grid',
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, alignSelf: 'center' }}>
          <div style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
            <div className="text-content" style={{ fontSize: 18, fontWeight: 600 }}>
              {fmt(baseTotal(e))}
            </div>
  ...
  function SettlementRow({ s }: { s: Settlement }) {
    return (
      ...
      >
        ...
        <div style={{ minWidth: 0 }}>
          <div className="text-content" style={{ fontSize: 15, fontWeight: 600, marginBottom: 6 }}>
            {t('costs.payment')}
          </div>
          <div
            style={{ display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }}
            title={`${personName(s.from_user_id)} → ${personName(s.to_user_id)}`}
          >
            <Avatar id={s.from_user_id} size={20} />
            <ArrowRight size={13} className="text-content-faint" />
            <Avatar id={s.to_user_id} size={20} />
            <span
              className="text-content-faint"
              style={{ fontSize: 12, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
            >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, alignSelf: 'center' }}>
          <div className="text-content" style={{ fontSize: 18, fontWeight: 600, whiteSpace: 'nowrap' }}>
            {fmt(s.amount)}
          </div>
          {canEdit && (
            <div className="exp-actions" style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
  ...
  function BalancesList({ balances }: { balances: SettlementData['balances'] }) {
    const rows = people.map(
      (p) =>
        balances.find((b) => b.user_id === p.id) || {
          user_id: p.id,
          username: p.username,
          avatar_url: null,
          balance: 0,
            ...
            >
              ...
              <div>
                ...
                <div
                  ...
                  style={{ height: 5, borderRadius: 3, marginTop: 5, position: 'relative', overflow: 'hidden' }}
                >
                  <span
                    style={{
                      position: 'absolute',
              ...
              <div
                style={{
                  ...
                  fontWeight: 600,
                  textAlign: 'right',
                  color: pos ? '#16a34a' : neg ? '#dc2626' : 'var(--text-faint)',
                }}
              >
  ...
  function CategoryBreakdown() {
    ...
    if (rows.length === 0)
      return (
        <div className="text-content-faint" style={{ fontSize: 12.5 }}>
        ...
        {rows.map((c) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-content" style={{ fontSize: 13, fontWeight: 500 }}>
                {t(c.labelKey)}
              </span>
              <span className="text-content-muted" style={{ fontSize: 13, fontWeight: 600 }}>
                {fmt0(v)}
              </span>
              <div
                className="bg-surface-secondary"
                style={{ gridColumn: '1 / -1', height: 5, borderRadius: 3, overflow: 'hidden', marginTop: -2 }}
              >
                <span
                  style={{ display: 'block', height: '100%', width: pct + '%', background: c.color, borderRadius: 3 }}
                />
              </div>
            </div>
          );
...
function SummaryCard({
  ...
  try {
    ...
    parts = new Intl.NumberFormat(currencyLocale(currency), {
      style: 'currency',
  ...
  return (
    <div
      className={total ? '' : 'border border-edge bg-surface-card'}
      style={{
        borderRadius: 22,
        padding: '26px 28px',
        position: 'relative',
        overflow: 'hidden',
        ...(total ? { background: 'linear-gradient(135deg,#1f2937,#111827)', color: '#fff' } : {}),
      }}
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
        <span
          style={{
            ...
            display: 'grid',
            placeItems: 'center',
            background: total ? 'rgba(255,255,255,0.12)' : accent + '22',
            color: total ? '#fff' : accent,
          }}
        >
          {icon}
        </span>
        <div>
          <div style={{ fontSize: 13, fontWeight: 600 }} className={total ? '' : 'text-content'}>
            {label}
          </div>
          <div style={{ fontSize: 12, opacity: total ? 0.6 : 1 }} className={total ? '' : 'text-content-faint'}>
            {sub}
          </div>
        </div>
      </div>
      ...
      >
        {parts ? (
          parts.map((p, i) => (
            <span key={i} style={big(p) ? undefined : { fontSize: 26, fontWeight: 500, color: muted }}>
              {p.value}
            </span>
          ))
        ) : (
...
function FlowPills({
  ...
  ids: number[];
  lead: string;
  Avatar: (p: { id: number; size?: number }) => React.JSX.Element;
    ...
    <Modal
      ...
      footer={
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          ...
          >
            {editing ? t('common.save') : t('costs.addPayment')}
          </button>
        </div>
      }
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div>
          ...
          <input
            ...
            className={inputCls}
            style={{ borderRadius: 10, padding: '11px 13px', fontSize: 14, outline: 'none', fontWeight: 600 }}
          />
        </div>
      </div>
    </Modal>
...
export function ExpenseModal({
  ...
  const rebalance = (
    ...
    const next = { ...paidMap };
    free.forEach((id, i) => {
      next[id] = shares[i] ? String(shares[i]) : '';
    });
    return next;
  };
    ...
    <Modal
      ...
      footer={
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          ...
          >
            {editing ? t('common.save') : t('costs.addExpense')}
          </button>
        </div>
      }
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div>
          ...
          <input
            ...
            className={inputCls}
            style={{ borderRadius: 10, padding: '11px 13px', fontSize: 14, outline: 'none' }}
          />
        </div>
        ...
        <div>
          ...
          >
            <span className="text-content-faint" style={{ fontSize: 15 }}>
              {sym(currency)}
            </span>
            <input
              type="number"
            ...
            {COST_CATEGORY_LIST.map((c) => {
              ...
              return (
                <button
                  ...
                  className={
                    on
                      ? 'border bg-surface-card text-content'
                      : 'border border-edge bg-surface-secondary text-content-muted'
                  }
                  style={{
                    display: 'inline-flex',
                    alignItems: 'center',
                    ...
                    cursor: 'pointer',
                    fontFamily: 'inherit',
                    borderColor: on ? 'var(--text-primary)' : undefined,
                  }}
                >
                  <span
                    style={{
        ...
        <div>
              ...
              return (
                ...
                >
                  ...
                  {on ? (
                    <div
                      className="border border-edge bg-surface-input"
                      style={{ display: 'flex', alignItems: 'center', gap: 4, borderRadius: 8, padding: '0 10px' }}
                    >
          ...
          >
            ...
            {paidEntered ? (
              <span style={{ fontWeight: 600, color: balanced ? '#16a34a' : '#dc2626' }}>
                {sym(currency)}
                {paidSum.toFixed(2)} / {sym(currency)}
                {totalNum.toFixed(2)}
              </span>
            ) : (
