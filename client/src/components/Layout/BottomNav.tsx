...
function useCreateAction(): { label: string; run: () => void } {
  ...
  if (inTrip) {
    ...
    const tripTab =
      typeof sessionStorage !== 'undefined' ? sessionStorage.getItem(`trip-tab-${inTrip.params.id}`) : null;
...
export default function BottomNav() {
  ...
  const renderItem = ({ to, label, icon: Icon }: NavItem) => {
    ...
    return (
      <button
        ...
        style={{
          color: active ? (dark ? '#fff' : 'oklch(0.22 0 0)') : dark ? 'oklch(0.6 0 0)' : 'oklch(0.62 0.01 65)',
        }}
      >
        <Icon size={21} strokeWidth={active ? 2.4 : 1.9} />
        <span className="max-w-full truncate text-[10px] font-semibold tracking-tight">{label}</span>
      </button>
    );
  };
  ...
  return (
    <nav
      className="fixed z-[60] flex items-center md:hidden glass-panel"
      style={{
        left: 12,
        right: 12,
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 items-center justify-around">{right.map(renderItem)}</div>
    </nav>
  );
}
