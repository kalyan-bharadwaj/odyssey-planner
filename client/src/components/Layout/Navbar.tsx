...
export default function Navbar({ tripTitle, tripId, onBack, showBack, onShare }: NavbarProps): React.ReactElement {
  ...
  const toggleDarkMode = () => {
    document.documentElement.classList.add('odyssey-theme-transitioning');
    ...
    themeTransitionTimer.current = window.setTimeout(() => {
      document.documentElement.classList.remove('odyssey-theme-transitioning');
      themeTransitionTimer.current = null;
    }, 360);
  };
  ...
  return (
    <nav
      ...
      className={`fixed left-0 right-0 top-0 z-[200] hidden items-center gap-4 px-4 md:flex ${scrolled ? 'glass-panel' : 'bg-transparent'}`}
    >
      {/* Left side */}
      <div className="flex min-w-0 items-center gap-3">
        {showBack && (
          <button
            onClick={onBack}
            className="odyssey-back-btn flex flex-shrink-0 items-center gap-1.5 rounded-lg p-1.5 text-sm text-content-muted transition-colors"
            onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--bg-hover)')}
            onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}
          >
            <ArrowLeft className="odyssey-back-icon h-4 w-4" />
      ...
      {globalAddons.length > 0 && !tripTitle && (
        <div
          className="odyssey-nav-pill glass-panel"
          style={{
            position: 'absolute',
            left: '50%',
            top: '50%',
          ...
          {userMenuOpen &&
            ReactDOM.createPortal(
              <>
                ...
                <div
                  className="odyssey-menu-enter w-52 overflow-hidden rounded-xl border border-edge bg-surface-card shadow-xl"
