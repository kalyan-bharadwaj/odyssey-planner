import type { TripCreateRequest } from '@odyssey/shared';
...
export default function TripFormModal({ isOpen, onClose, onSave, trip, onCoverUpdate }: TripFormModalProps) {
  ...
  useEffect(() => {
    ...
    if (trip) {
      tripsApi
        .getMembers(trip.id)
        .then((d) => setExistingMembers(d.members || []))
        .catch(() => {});
    } else {
      setExistingMembers([]);
    }
    ...
    try {
      ...
      if (selectedMembers.length > 0 && createdTrip?.id) {
        ...
        for (const userId of selectedMembers) {
          ...
          if (user) {
            try {
              await tripsApi.addMember(createdTrip.id, user.username);
            } catch {
  ...
  return (
      ...
      footer={
        <div className="flex justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg border border-slate-200 px-4 py-2 text-sm text-slate-600 transition-colors hover:bg-slate-50 hover:text-slate-800"
          >
            {t('common.cancel')}
          </button>
          <button
            ...
            disabled={isLoading}
            className="flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm text-white transition-colors hover:bg-slate-700 disabled:bg-slate-400"
          >
            {isLoading ? (
              <>
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4" onPaste={handlePaste}>
        ...
        {canUploadCover && (
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-700">{t('dashboard.coverImage')}</label>
            <input
              ref={fileRef}
              type="file"
              accept="image/*"
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => canEditTrip && update('title', e.target.value)}
            required
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-slate-700">{t('dashboard.tripDescription')}</label>
          <textarea
            value={formData.description}
            onChange={(e) => canEditTrip && update('description', e.target.value)}
            readOnly={!canEditTrip}
        ...
        {!formData.start_date && !formData.end_date && (
          <div>
            <label className="mb-1.5 block text-sm font-medium text-slate-700">{t('dashboard.dayCount')}</label>
            <input
              type="number"
              min={1}
              max={365}
              ...
              className={inputCls}
            />
            <p className="mt-1.5 text-xs text-slate-400">{t('dashboard.dayCountHint')}</p>
          </div>
        )}
        ...
        {(!isEditing || trip?.user_id === currentUser?.id || currentUser?.role === 'admin') && (
          <div className={!tripRemindersEnabled ? 'opacity-50' : ''}>
            ...
            {!tripRemindersEnabled ? (
              <p className="rounded-lg bg-slate-50 p-3 text-xs text-slate-400">{t('trips.reminderDisabledHint')}</p>
            ) : (
              <>
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        update('reminder_days', opt.value);
                        setCustomReminder(false);
                      }}
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if ([0, 1, 3, 9].includes(formData.reminder_days)) update('reminder_days', 7);
                    }}
                    className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition-colors ${
                      customReminder
                        ? 'border-slate-900 bg-slate-900 text-white'
                ...
                {customReminder && (
                  <div className="mt-2 flex items-center gap-2">
                    <input
                      ...
                      className="w-20 rounded-lg border border-slate-200 px-3 py-1.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-300"
                    />
                  ...
                  <span
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        toast.error(t('trips.memberRemoveError'));
                      }
                    }}
                    style={{
                      display: 'flex',
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
                {selectedMembers.map((uid) => {
                  ...
                  return (
                    <span
                      key={uid}
                      onClick={() => setSelectedMembers((prev) => prev.filter((id) => id !== uid))}
                      className="cursor-pointer border border-edge bg-surface-secondary text-content"
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: 5,
