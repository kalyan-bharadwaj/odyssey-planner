...
describe('CollabNotes', () => {
  ...
  it('FE-COMP-NOTES-039: NoteFormModal website field accepts URL input', async () => {
    ...
    await user.type(websiteInput, 'https://odyssey.app');
    await user.click(screen.getByRole('button', { name: /^Create$/i }));
    await waitFor(() => expect(postBody.website).toBe('https://odyssey.app'));
  });
  ...
  it('FE-COMP-NOTES-050: WebsiteThumbnail with OG image renders thumbnail image', async () => {
    server.use(
          ...
          notes: [
            {
              ...
              category: null,
              color: '#3b82f6',
              website: 'https://odyssey-app.example.com',
              files: [],
              attachments: [],
              created_at: '2025-06-01T10:00:00.000Z',
              updated_at: '2025-06-01T10:00:00.000Z',
      ...
      http.get('/api/trips/1/collab/link-preview', () =>
        HttpResponse.json({ title: 'Odyssey App', image: 'https://odyssey-app.example.com/og.jpg' })
      )
    );
    render(<CollabNotes {...defaultProps} />);
