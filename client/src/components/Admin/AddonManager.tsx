  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-xl border border-edge bg-surface-card">
        <div className="border-b border-edge-secondary px-6 py-4">
        ...
        {addons.length === 0 ? (
          <div className="p-8 text-center text-sm text-content-faint">{t('admin.addons.noAddons')}</div>
        ) : (
          <div>
            {/* Trip Addons */}
            {tripAddons.length > 0 && (
              <div>
                <div className="flex items-center gap-2 border-b border-edge-secondary bg-surface-secondary px-6 py-2.5">
                ...
                {tripAddons.map((addon) => (
                  <div key={addon.id}>
                    ...
                    {addon.id === 'packing' && addon.enabled && onToggleBagTracking && (
                      <div
                        className="flex items-center gap-4 border-b border-edge-secondary bg-surface-secondary px-6 py-3"
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div className="mt-0.5 text-xs text-content-faint">{t('admin.bagTracking.subtitle')}</div>
                        </div>
                        <div className="flex shrink-0 items-center gap-2">
                          <span
                            className={`hidden text-xs font-medium sm:inline ${bagTrackingEnabled ? 'text-content' : 'text-content-faint'}`}
                          >
                    ...
                    {addon.id === 'collab' && addon.enabled && collabFeatures && onToggleCollabFeature && (
                      <div
                        className="border-b border-edge-secondary bg-surface-secondary px-6 py-3"
                        style={{ paddingLeft: 70 }}
                      >
                            ...
                            return (
                              <div key={feat.key} className="flex items-center gap-4" style={{ minHeight: 32 }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  <div className="mt-0.5 text-xs text-content-faint">{t(feat.subtitleKey)}</div>
                                </div>
                                <div className="flex shrink-0 items-center gap-2">
                                  <span
                                    className={`hidden text-xs font-medium sm:inline ${enabled ? 'text-content' : 'text-content-faint'}`}
                                  >
            ...
            {globalAddons.length > 0 && (
              <div>
                <div className="flex items-center gap-2 border-b border-t border-edge-secondary bg-surface-secondary px-6 py-2.5">
                            ...
                            return (
                              <div key={provider.key} className="flex items-center gap-4" style={{ minHeight: 32 }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  <div className="mt-0.5 text-xs text-content-faint">{provider.description}</div>
                                </div>
                                <div className="flex shrink-0 items-center gap-2">
                                  <span
                                    className={`hidden text-xs font-medium sm:inline ${provider.enabled ? 'text-content' : 'text-content-faint'}`}
                                  >
...
function AddonRow({
  ...
  return (
    <div
      className="flex items-center gap-4 border-b border-edge-secondary px-6 py-4 transition-colors hover:opacity-95"
      style={{ opacity: isComingSoon ? 0.5 : 1, pointerEvents: isComingSoon ? 'none' : 'auto' }}
    >
      {/* Icon */}
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-surface-secondary text-content">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          {isComingSoon && (
            <span className="rounded-full bg-surface-tertiary px-2 py-0.5 text-[9px] font-semibold text-content-faint">
              Coming Soon
            </span>
          )}
          <span className="rounded-full bg-surface-secondary px-1.5 py-0.5 text-[10px] font-medium text-content-muted">
            {addon.type === 'global'
        ...
        <p className="mt-0.5 text-xs text-content-muted">{displayDescription}</p>
      </div>

      {/* Toggle */}
      <div className="flex shrink-0 items-center gap-2">
        <span
          className={`hidden text-xs font-medium sm:inline ${enabledState && !isComingSoon ? 'text-content' : 'text-content-faint'}`}
        >
          {isComingSoon
