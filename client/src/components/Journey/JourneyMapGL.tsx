...
function ensureJourneyPopupStyle() {
  if (document.getElementById('odyssey-journey-popup-style')) return;
  const s = document.createElement('style');
  s.id = 'odyssey-journey-popup-style';
  s.textContent = `
    .mapboxgl-popup.odyssey-journey-popup { pointer-events: none; animation: odyssey-journey-popup-in 180ms ease-out; }
    .mapboxgl-popup.odyssey-journey-popup .mapboxgl-popup-content {
    ...
    .mapboxgl-popup.odyssey-journey-popup.odyssey-dark .mapboxgl-popup-content {
      background: rgba(24, 24, 27, 0.88);
      border-color: rgba(255, 255, 255, 0.08);
      color: #FAFAFA;
    }
    .mapboxgl-popup.odyssey-journey-popup .mapboxgl-popup-tip {
      border-top-color: rgba(255, 255, 255, 0.94);
      border-bottom-color: rgba(255, 255, 255, 0.94);
    }
    .mapboxgl-popup.odyssey-journey-popup.odyssey-dark .mapboxgl-popup-tip {
      border-top-color: rgba(24, 24, 27, 0.88);
      border-bottom-color: rgba(24, 24, 27, 0.88);
    }
    .mapboxgl-popup.odyssey-journey-popup .mapboxgl-popup-close-button { display: none; }
    .odyssey-journey-popup-title {
      font-size: 13.5px;
      font-weight: 600;
    ...
    .mapboxgl-popup.odyssey-journey-popup.odyssey-dark .odyssey-journey-popup-title { color: #FAFAFA; }
    .odyssey-journey-popup-sub {
      display: flex;
    ...
    .mapboxgl-popup.odyssey-journey-popup.odyssey-dark .odyssey-journey-popup-sub { color: #A1A1AA; }
    .odyssey-journey-popup-place {
      min-width: 0;
      overflow: hidden;
      text-overflow: ellipsis;
    }
    .odyssey-journey-popup-sep {
      flex: 0 0 auto;
      opacity: 0.55;
      font-weight: 500;
    }
    .odyssey-journey-popup-date { flex: 0 0 auto; }
    @keyframes odyssey-journey-popup-in {
      from { opacity: 0; }
      to { opacity: 1; }
    }
  `;
...
function markerHtml(dayColor: string, dayLabel: number, highlighted: boolean): HTMLDivElement {
  ...
  inner.className = 'odyssey-journey-marker-inner';
...
const JourneyMapGL = forwardRef<JourneyMapGLHandle, Props>(function JourneyMapGL(
  ...
  const showPopup = useCallback((id: string) => {
    ...
    if (place) subParts.push(`<span class="odyssey-journey-popup-place">${place}</span>`);
    if (date) subParts.push(`<span class="odyssey-journey-popup-date">${date}</span>`);
    const subline =
      subParts.length === 2
        ? `${subParts[0]}<span class="odyssey-journey-popup-sep">\u00B7</span>${subParts[1]}`
        : subParts.join('');

    const html = `
      <div class="odyssey-journey-popup-title">${primary}</div>
      ${subline ? `<div class="odyssey-journey-popup-sub">${subline}</div>` : ''}
    `;
    ...
    if (popupRef.current) {
      ...
      if (el) el.classList.toggle('odyssey-dark', !!darkRef.current);
    } else {
      popupRef.current = new mapboxgl.Popup({
        ...
        anchor: 'bottom',
        offset,
        className: `odyssey-journey-popup${darkRef.current ? ' odyssey-dark' : ''}`,
        maxWidth: '280px',
      })
  ...
  const setMarkerStyle = useCallback((id: string, highlighted: boolean) => {
    ...
    const currentInner = el.querySelector('.odyssey-journey-marker-inner') as HTMLDivElement | null;
    ...
    const nextInner = next.querySelector('.odyssey-journey-marker-inner') as HTMLDivElement;
