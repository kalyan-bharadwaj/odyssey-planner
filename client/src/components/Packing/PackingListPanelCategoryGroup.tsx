...
import type { CategoryAssignee, TripMember } from './usePackingListPanel';
...
export function KategorieGruppe({
  ...
  const handleSaveKatName = async () => {
    ...
    if (!neu || neu === kategorie) {
      ...
      setEditKatName(kategorie);
      return;
    }
    try {
      await onRename(kategorie, neu);
      setEditingName(false);
    } catch {
    ...
    >
      <div
        style={{
          ...
          gap: 8,
          padding: '10px 12px',
          borderBottom: offen ? '1px solid var(--border-secondary)' : 'none',
        }}
      >
        <button
        ...
        {editingName && canEdit ? (
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') handleSaveKatName();
              if (e.key === 'Escape') {
                setEditingName(false);
                setEditKatName(kategorie);
              }
            ...
            <div
              ...
              onClick={(e) => {
                ...
                if (canEdit)
                  onSetAssignees(
                    kategorie,
                    assignees.filter((x) => x.user_id !== a.user_id).map((x) => x.user_id)
                  );
              }}
            >
              <div
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 8px' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 8,
                    border: 'none',
                    background: newItemName.trim() ? 'var(--text-primary)' : 'var(--border-primary)',
                    color: 'var(--bg-primary)',
