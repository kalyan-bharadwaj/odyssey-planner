...
export function ArtikelZeile({
  ...
  const handleSaveName = async () => {
    if (!editName.trim()) {
      setEditing(false);
      setEditName(isPlaceholder ? '' : item.name);
      return;
    }
    try {
      await updatePackingItem(tripId, item.id, { name: editName.trim() });
      setEditing(false);
    } catch {
  ...
  const handleCatChange = async (cat: string) => {
    ...
    try {
      await updatePackingItem(tripId, item.id, { category: cat });
    } catch {
    ...
    >
      ...
      {canEdit && (
        <QuantityInput
          ...
          onSave={(qty) => updatePackingItem(tripId, item.id, { quantity: qty })}
        />
      )}
          ...
          >
            <input
              ...
              onChange={async (e) => {
                ...
                try {
                  await updatePackingItem(tripId, item.id, { weight_grams: v });
                } catch {
                  toast.error(t('packing.toast.saveError'));
                }
              }}
          ...
          <div style={{ position: 'relative' }}>
            <button
              ...
              style={{
                ...
                border: item.bag_id
                  ? `2.5px solid ${bags.find((b) => b.id === item.bag_id)?.color || 'var(--border-primary)'}`
                  : '2px dashed var(--border-primary)',
                background: item.bag_id
                  ? `${bags.find((b) => b.id === item.bag_id)?.color || 'var(--border-primary)'}30`
                  : 'transparent',
              }}
            >
                ...
                {item.bag_id && (
                  <button
                    onClick={async () => {
                      setShowBagPicker(false);
                      try {
                        await updatePackingItem(tripId, item.id, { bag_id: null });
                      } catch {
                        toast.error(t('packing.toast.saveError'));
                      }
                    }}
                  ...
                  >
                    <span
                      style={{ width: 10, height: 10, borderRadius: '50%', border: '2px dashed var(--border-primary)' }}
                    />
                    {t('packing.noBag')}
                  </button>
                )}
                {bags.map((b) => (
                  <button
                    ...
                    onClick={async () => {
                      setShowBagPicker(false);
                      try {
                        await updatePackingItem(tripId, item.id, { bag_id: b.id });
                      } catch {
                        toast.error(t('packing.toast.saveError'));
                      }
                    }}
                    ...
                    onMouseEnter={(e) => {
                      if (item.bag_id !== b.id) e.currentTarget.style.background = 'var(--bg-tertiary)';
                    }}
                    onMouseLeave={(e) => {
                      if (item.bag_id !== b.id) e.currentTarget.style.background = 'none';
                    }}
                  >
                  ...
                  {bagInlineCreate ? (
                    <div style={{ display: 'flex', gap: 4 }}>
                      <input
                        autoFocus
                        value={bagInlineName}
                        onChange={(e) => setBagInlineName(e.target.value)}
                        onKeyDown={async (e) => {
                          if (e.key === 'Enter' && bagInlineName.trim()) {
                            const newBag = await onCreateBag(bagInlineName.trim());
                            if (newBag) {
                              try {
                                await updatePackingItem(tripId, item.id, { bag_id: newBag.id });
                              } catch {
                                toast.error(t('packing.toast.saveError'));
                              }
                            }
                            setBagInlineName('');
                            setBagInlineCreate(false);
                            setShowBagPicker(false);
                          }
                          if (e.key === 'Escape') {
                            setBagInlineCreate(false);
                    ...
                    <button
                      onClick={() => setBagInlineCreate(true)}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: 5,
      ...
      {canEdit && (
        <div
          ...
          style={{ display: 'flex', gap: 2, alignItems: 'center', transition: 'opacity 0.12s', flexShrink: 0 }}
        >
          <div style={{ position: 'relative' }}>
            <button
            ...
            >
              <span
                style={{
                  ...
                  height: 7,
                  borderRadius: '50%',
                  background: katColor(item.category || t('packing.defaultCategory'), categories),
                ...
                {categories.map((cat) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: '6px 10px',
                      background:
                        cat === (item.category || t('packing.defaultCategory')) ? 'var(--bg-tertiary)' : 'none',
                      border: 'none',
                      cursor: 'pointer',
                      fontSize: 12.5,
                      fontFamily: 'inherit',
                  ...
                  >
                    <span
                      style={{
                        ...
                        height: 8,
                        borderRadius: '50%',
                        background: katColor(cat, categories),
                        flexShrink: 0,
                      }}
                    />
                    {cat}
