...
export const MemoPlaceRow = React.memo(function MemoPlaceRow({
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        if (!isSelected && !isChecked) e.currentTarget.style.background = 'var(--bg-hover)';
      }}
      onMouseLeave={(e) => {
    ...
    >
      {selectMode && (
        <div
          className={isChecked ? 'bg-accent' : 'bg-transparent'}
          style={{
            width: 16,
            height: 16,
            borderRadius: 4,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, overflow: 'hidden' }}>
          {hasGeometry && (
            <span title="Track / Route" style={{ display: 'inline-flex', flexShrink: 0 }}>
              <Route size={11} strokeWidth={2} color="var(--text-faint)" />
            </span>
          )}
          {cat &&
            (() => {
