...
export function ReservationModal({
  ...
  const [form, setForm] = useState({
    ...
    confirmation_number: '',
    notes: '',
    assignment_id: '' as string | number,
    accommodation_id: '' as string | number,
    meta_check_in_time: '',
    meta_check_in_end_time: '',
    meta_check_out_time: '',
    hotel_place_id: '' as string | number,
  ...
  useEffect(() => {
    if (reservation) {
      ...
      setForm({
        ...
        hotel_start_day: (() => {
          const acc = accommodations.find((a) => a.id == reservation.accommodation_id);
          return acc?.start_day_id || '';
        })(),
        hotel_end_day: (() => {
          const acc = accommodations.find((a) => a.id == reservation.accommodation_id);
          return acc?.end_day_id || '';
        })(),
      });
    } else {
      setForm({
  ...
  const handleSubmit = async (e?: { preventDefault?: () => void }) => {
    ...
    if (isEndBeforeStart) {
      toast.error(t('reservations.validation.endBeforeStart'));
      return;
    }
    setIsSaving(true);
    try {
      ...
      footer={
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          ...
          <button
            ...
            disabled={isSaving || !form.title.trim() || isEndBeforeStart}
            ...
            style={{
              ...
              cursor: 'pointer',
              fontFamily: 'inherit',
              opacity: isSaving || !form.title.trim() || isEndBeforeStart ? 0.5 : 1,
            }}
          >
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        {form.type !== 'hotel' && (
          <>
            ...
            {isEndBeforeStart && (
              <div className="text-[#ef4444]" style={{ fontSize: 11, marginTop: -6 }}>
          ...
          <>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div>
                ...
                <CustomSelect
                  ...
                  options={[{ value: '', label: '—' }, ...places.map((p) => ({ value: p.id, label: p.name }))]}
                  searchable
                  size="sm"
                />
              </div>
                ...
                <CustomSelect
                  ...
                  onChange={(value) =>
                    setForm((prev) => ({
                      ...
                      hotel_end_day:
                        days.findIndex((d) => d.id === value) > days.findIndex((d) => d.id === prev.hotel_end_day)
                          ? value
                          : prev.hotel_end_day,
                    }))
                  }
                ...
                <CustomSelect
                  ...
                  onChange={(value) =>
                    setForm((prev) => ({
                      ...prev,
                      hotel_start_day:
                        days.findIndex((d) => d.id === value) < days.findIndex((d) => d.id === prev.hotel_start_day)
                          ? value
                          : prev.hotel_start_day,
                      hotel_end_day: value,
                    }))
                  }
                  placeholder={t('reservations.meta.selectDay')}
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              ...
              <div>
                ...
                <CustomTimePicker
                  value={form.meta_check_in_end_time}
                  onChange={(v) => set('meta_check_in_end_time', v)}
                />
              </div>
              <div>
