...
import type { AssignmentsMap, Day, Reservation, TripFile } from '../../types';
...
function ReservationCard({
  ...
  const attachedFiles = files.filter(
    (f) => f.reservation_id === r.id || (f.linked_reservation_ids || []).includes(r.id)
  );
  ...
  const handleToggle = async () => {
    try {
      await toggleReservationStatus(tripId, r.id);
    } catch {
  ...
  const startDay = r.day_id
    ? days.find((d) => d.id === r.day_id)
    : isHotel && r.accommodation_start_day_id
      ? days.find((d) => d.id === r.accommodation_start_day_id)
      : undefined;
  const endDay = r.end_day_id
    ? days.find((d) => d.id === r.end_day_id)
    : isHotel && r.accommodation_end_day_id
      ? days.find((d) => d.id === r.accommodation_end_day_id)
      : undefined;
  const DayLabel = ({ day }: { day: typeof startDay }) => {
    ...
    return (
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
        ...
        {badge && (
          <span
            className="bg-surface-secondary text-content-faint"
            style={{
              fontSize: 10,
              fontWeight: 600,
              padding: '1px 6px',
  ...
  return (
    ...
    >
      ...
      <div
        className={confirmed ? 'bg-[rgba(22,163,74,0.06)]' : 'bg-[rgba(217,119,6,0.06)]'}
        style={{
          display: 'flex',
          alignItems: 'center',
      ...
      >
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, minWidth: 0, flexWrap: 'wrap' }}>
          <span
            className={confirmed ? 'text-[#16a34a]' : 'text-[#d97706]'}
            style={{
              display: 'inline-flex',
              alignItems: 'center',
              gap: 6,
          ...
          >
            <span
              className={confirmed ? 'bg-[#16a34a]' : 'bg-[#d97706]'}
          ...
          {r.needs_review ? (
            <span
              className="bg-[rgba(245,158,11,0.12)] text-[#b45309]"
              style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: 4,
          ...
          {r.external_source === 'airtrail' ? (
            <span
              className={
                r.sync_enabled ? 'bg-[rgba(59,130,246,0.12)] text-[#2563eb]' : 'bg-surface-tertiary text-content-faint'
              }
              style={{
                display: 'inline-flex',
                alignItems: 'center',
      ...
      <div style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 12, flex: 1 }}>
            ...
            {hasDate && (
              <div>
                ...
                <div className={`${fieldValueClass} text-center`}>
                  ...
                  {endDt.date && endDt.date !== startDt.date && <> – {fmtDate(endDt.date)}</>}
                </div>
              </div>
            )}
            {hasTime && (
        ...
        {(() => {
          ...
          const hasEndpoints =
            (r.endpoints || []).some((e) => e.role === 'from') && (r.endpoints || []).some((e) => e.role === 'to');
          ...
          if (!hasEndpoints && meta.departure_airport)
            cells.push({ label: t('reservations.meta.from'), value: meta.departure_airport });
          if (!hasEndpoints && meta.arrival_airport)
            cells.push({ label: t('reservations.meta.to'), value: meta.arrival_airport });
          ...
          if (meta.check_in_time)
            cells.push({
              ...
              value:
                formatTime(meta.check_in_time, locale, timeFormat) +
                (meta.check_in_end_time ? ` – ${formatTime(meta.check_in_end_time, locale, timeFormat)}` : ''),
            });
          if (meta.check_out_time)
            cells.push({
              label: t('reservations.meta.checkOut'),
              value: formatTime(meta.check_out_time, locale, timeFormat),
            });
          ...
          return (
            <div
              style={{
                display: 'grid',
                gap: 10,
                gridTemplateColumns: cells.length > 1 ? `repeat(${Math.min(cells.length, 3)}, 1fr)` : '1fr',
              }}
            >
              {cells.map((c, i) => (
                <div key={i}>
        ...
        {r.notes && (
          <div>
            ...
            <div
              className={`collab-note-md ${fieldValueClass}`}
              style={{ fontWeight: 400, lineHeight: 1.5, wordBreak: 'break-word', overflowWrap: 'anywhere' }}
            >
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div
                  className="bg-[rgba(239,68,68,0.12)]"
                  style={{
                    width: 36,
                    height: 36,
                    flexShrink: 0,
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      ...
      >
        ...
        <span
          className="text-content-muted"
          style={{ fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.08em' }}
        >
          {title}
        </span>
        <span
      ...
      {open && (
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: 'repeat(auto-fill, minmax(max(33.33% - 14px, 340px), 1fr))',
            gap: 14,
            alignItems: 'stretch',
          }}
        >
...
export default function ReservationsPanel({
  ...
  addManualKey = 'reservations.addManual',
  ...
  const storageKey = `odyssey-reservation-filters-${tripId}`;
          ...
          {reservations.length > 0 && (
            <>
              <div
                ...
                style={{ width: 1, height: 22, background: 'var(--border-faint)', flexShrink: 0 }}
              />
              <div className="hidden md:inline-flex" style={{ gap: 4, flexWrap: 'wrap', flex: 1, minWidth: 0 }}>
                <button
                  ...
                  className={
                    typeFilters.size === 0 ? 'bg-surface-card text-content' : 'bg-transparent text-content-muted'
                  }
                  style={{
                    appearance: 'none',
                    border: 'none',
                ...
                >
                  {t('common.all')}
                  <span
                    className={`text-content-faint ${typeFilters.size === 0 ? 'bg-surface-tertiary' : 'bg-[rgba(0,0,0,0.06)]'}`}
                    style={{
                      fontSize: 10,
                      fontWeight: 600,
                      padding: '1px 6px',
          ...
          {canEdit && (
            <div style={{ display: 'flex', gap: 6, marginLeft: 'auto', flexShrink: 0 }}>
              ...
              {onAirTrailImport && airTrailAvailable && (
                <button
                  onClick={onAirTrailImport}
                  className="bg-surface-secondary text-content"
                  style={{
                    appearance: 'none',
                    border: '1px solid var(--border-primary)',
                    cursor: 'pointer',
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px 80px' }} className="max-md:!px-4 max-md:!pt-4">
        {total === 0 && reservations.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <p className="text-content-secondary" style={{ fontSize: 14, fontWeight: 600, margin: '0 0 4px' }}>
              {t('reservations.empty')}
            </p>
            <p className="text-content-faint" style={{ fontSize: 12, margin: 0 }}>
              {t('reservations.emptyHint')}
            </p>
          </div>
        ) : total === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            <p className="text-content-faint" style={{ fontSize: 13 }}>
              {t('places.noneFound')}
            </p>
          </div>
        ) : (
          <>
            {allPending.length > 0 && (
              <Section
                ...
                accent="gray"
                storageKey={`odyssey:bookings-pending-open:${tripId}`}
              >
                {allPending.map((r) => (
                  <ReservationCard
                    key={r.id}
            ...
            {allConfirmed.length > 0 && (
              <Section
                ...
                accent="green"
                storageKey={`odyssey:bookings-confirmed-open:${tripId}`}
              >
                {allConfirmed.map((r) => (
                  <ReservationCard
                    key={r.id}
