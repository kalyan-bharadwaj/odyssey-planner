...
import type { AssignmentsMap, Category, Day, Place, Reservation, TripFile } from '../../types';
...
export default function PlaceInspector({
  ...
  const placeFiles = (files || []).filter(
    (f) => String(f.place_id) === String(place.id) || (f.linked_place_ids || []).includes(place.id)
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="hidden sm:flex" style={{ flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
                  ...
                  <Chip
                    ...
                    text={
                      <>
                        ...
                        {googleDetails.rating_count ? (
                          <span style={{ opacity: 0.5 }}> ({googleDetails.rating_count.toLocaleString(locale)})</span>
                        ) : (
                          ''
                        )}
                        {shortReview && (
            ...
            {place.price > 0 && (
              <Chip
                icon={<Euro size={12} />}
                text={`${place.price} ${place.currency || '€'}`}
                color="#059669"
                bg="#ecfdf5"
              />
            )}
          ...
          {(place.description || googleDetails?.summary) && (
            <div
              className="collab-note-md bg-surface-hover text-content-muted"
              style={{
                borderRadius: 10,
                overflow: 'hidden',
                flexShrink: 0,
        ...
        {/* Footer actions */}
        <div
          className="border-t border-edge-faint"
          style={{
            padding: '10px 16px',
            display: 'flex',
            gap: 6,
        ...
        >
          {selectedDayId &&
            (assignmentInDay ? (
              <ActionButton
                ...
                label={
                  <>
                    <span className="hidden sm:inline">{t('inspector.removeFromDay')}</span>
                    <span className="sm:hidden">{t('inspector.remove')}</span>
                  </>
                }
              />
            ) : (
          ...
          {googleDetails?.google_maps_url && (
            <ActionButton
              onClick={() => window.open(googleDetails.google_maps_url, '_blank')}
              variant="ghost"
          ...
          {(place.website || googleDetails?.website) && (
            <ActionButton
              onClick={() => window.open(place.website || googleDetails?.website, '_blank')}
              variant="ghost"
          ...
          <ActionButton
            ...
            variant="ghost"
            icon={<Edit2 size={13} />}
            label={<span className="hidden sm:inline">{t('common.edit')}</span>}
          />
          <ActionButton
            onClick={onDelete}
            variant="danger"
            icon={<Trash2 size={13} />}
            label={<span className="hidden sm:inline">{t('common.delete')}</span>}
          />
        </div>
      </div>
    </div>
        ...
        {activeMembers.map((member) => {
          ...
          return (
            <div
              ...
              onClick={() => {
                if (canRemove) handleRemove(member.id);
              }}
              className={
                isHovered && canRemove ? 'bg-[rgba(239,68,68,0.06)] text-[#ef4444]' : 'bg-surface-hover text-content'
              }
              style={{
                display: 'flex',
                alignItems: 'center',
            ...
            >
              ...
              >
                {member.avatar_url || member.avatar ? (
                  <img
                    src={member.avatar_url || `/uploads/avatars/${member.avatar}`}
              ...
              <span style={{ textDecoration: isHovered && canRemove ? 'line-through' : 'none' }}>
                {member.username}
              </span>
            </div>
          );
  ...
  return (
    <div
      style={{
        display: 'flex',
        alignItems: 'center',
        gap: openNow !== null ? 26 : 14,
        padding: openNow !== null ? '18px 16px 14px 28px' : '18px 16px 14px',
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {place.lat && place.lng && (
          <div
            className="hidden text-content-faint sm:block"
            style={{ fontSize: 11, marginTop: 4, fontVariantNumeric: 'tabular-nums' }}
          >
...
function PlaceReservationParticipants({
  selectedAssignmentId,
  reservations,
  assignments,
  selectedDayId,
    ...
    <>
      {(() => {
        ...
        const assignment = selectedAssignmentId
          ? (assignments[String(selectedDayId)] || []).find((a) => a.id === selectedAssignmentId)
          : null;
                ...
                return (
                  <div
                    style={{
                      borderRadius: 12,
                      overflow: 'hidden',
                      border: `1px solid ${confirmed ? 'rgba(22,163,74,0.2)' : 'rgba(217,119,6,0.2)'}`,
                    }}
                  >
                    <div
                      className={confirmed ? 'bg-[rgba(22,163,74,0.08)]' : 'bg-[rgba(217,119,6,0.08)]'}
                          ...
                          <>
                            {date && (
                              <div>
                                <div
                                  className="text-content-faint"
                                  style={{ fontSize: 8, fontWeight: 600, textTransform: 'uppercase' }}
                                >
                                  {t('reservations.date')}
                                </div>
                    ...
                    {res.notes && (
                      <div
                        className="collab-note-md text-content-faint"
                        style={{
                          padding: '0 10px 6px',
                          fontSize: 10,
                          lineHeight: 1.4,
                    ...
                    {(() => {
                      ...
                      if (meta.departure_airport && meta.arrival_airport)
                        parts.push(`${meta.departure_airport} → ${meta.arrival_airport}`);
                      ...
                      return (
                        <div
                          className="text-content-muted"
                          style={{ padding: '0 10px 6px', fontSize: 10, fontWeight: 500 }}
                        >
                          {parts.join(' · ')}
                        </div>
                      );
          ...
          {hoursExpanded && (
            <div style={{ padding: '0 12px 10px' }}>
              {openingHours.map((line, i) => (
                <div
                  key={i}
                  className={i === weekdayIndex ? 'text-content' : 'text-content-muted'}
                  style={{
                    fontSize: 12,
              ...
              {placeFiles.map((f) => (
                ...
                >
                  {(f.mime_type || '').startsWith('image/') ? (
                    <FileImage size={12} color="#6b7280" />
                  ) : (
                  ...
                  {f.file_size && (
                    <span className="text-content-faint" style={{ fontSize: 11, flexShrink: 0 }}>
