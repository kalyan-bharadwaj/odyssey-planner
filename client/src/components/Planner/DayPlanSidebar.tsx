...
import {
  TRANSPORT_TYPES,
  getMergedItems as _getMergedItems,
  getTransportForDay as _getTransportForDay,
...
function useDayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  const {
    ...
    noteInputRef,
    dayNotes,
    openAddNote: _openAddNote,
    openEditNote: _openEditNote,
    cancelNote,
    saveNote,
  ...
  const setDropTargetKey = (key) => {
    dropTargetRef.current = key;
    _setDropTargetKey(key);
  };
  ...
  useEffect(() => {
    if (days.length > prevDayCount.current) {
      ...
      setExpandedDays((prev) => {
        ...
        days.forEach((d) => {
          if (!prev.has(d.id)) n.add(d.id);
        });
        try {
          sessionStorage.setItem(`day-expanded-${tripId}`, JSON.stringify([...n]));
        } catch {}
        return n;
      });
    }
  ...
  useEffect(() => {
    days.forEach((day) => initTransportPositions(day.id));
  }, [days, reservations]);
  ...
  const getTransportForDay = (dayId: number) =>
    _getTransportForDay({
      reservations,
      dayId,
      dayAssignmentIds: (assignments[String(dayId)] || []).map((a) => a.id),
      days,
    });
  ...
  const initTransportPositions = (dayId) => {
    ...
    const sorted = [...needsInit].sort(
      (a, b) => (parseTimeToMinutes(a.reservation_time) ?? 0) - (parseTimeToMinutes(b.reservation_time) ?? 0)
    );
  ...
  const mergedItemsMap = useMemo(() => {
    ...
    days.forEach((day) => {
      map[day.id] = getMergedItems(day.id);
    });
    return map;
  ...
  useEffect(() => {
    ...
    if (runs.length === 0 && !wantTop && !wantBottom) {
      setRouteLegs({});
      setHotelLegs({});
      return;
    }
    ...
    try {
      ...
      if (legResIds.length) {
        for (const ridStr of legResIds) {
          ...
          useTripStore.setState((state) => ({
            reservations: state.reservations.map((x) => (x.id === rid ? { ...x, metadata: newMeta } : x)),
          }));
  ...
  const handleMergedDrop = async (dayId, fromType, fromId, toType, toId, insertAfter = false, toLegIndex = null) => {
    ...
    const matchTo = (i: any) =>
      i.type === toType && i.data.id === toId && (toLegIndex == null || i.data?.__leg?.index === toLegIndex);
  ...
  const handleOptimize = async (dayId: number | null = selectedDayId) => {
    ...
    const anchors =
      day && useSettingsStore.getState().settings.optimize_from_accommodation !== false
        ? getAccommodationAnchors(day, days, accommodations)
        : {};
    const optimizedAssignments =
      unlockedWithCoords.length >= 2
        ? optimizeRoute(
            unlockedWithCoords.map((a) => ({ ...a.place, _assignmentId: a.id })),
            anchors
          )
            .map((p) => unlockedWithCoords.find((a) => a.id === p._assignmentId))
            .filter(Boolean)
        : unlockedWithCoords;
  ...
  const handleDropOnDay = (e, dayId) => {
    ...
    if (fromReservationId && fromDayId !== dayId) {
      ...
      if (r) {
        const update = computeMultiDayMove(r, dayId, phase);
        tripActions
          .updateReservation(tripId, r.id, update)
    ...
    if (placeId) {
      ...
      const srcAssignment = (useTripStore.getState().assignments[String(fromDayId)] || []).find(
        (a) => a.id === Number(assignmentId)
      );
      const capturedFromDayId = fromDayId;
      const capturedOrderIndex = srcAssignment?.order_index ?? 0;
      tripActions
        .moveAssignment(tripId, Number(assignmentId), fromDayId, dayId)
        .then(() => {
          pushUndo?.(t('undo.moveDay'), async () => {
            await tripActions.moveAssignment(
              tripId,
              Number(assignmentId),
              dayId,
              capturedFromDayId,
      ...
      tripActions
        .moveDayNote(tripId, fromDayId, dayId, Number(noteId))
        .catch((err: unknown) => toast.error(err instanceof Error ? err.message : t('common.unknownError')));
    }
    setDraggingId(null);
    setDropTargetKey(null);
  ...
  const anyGeoAssignment = Object.values(assignments)
    .flatMap((da) => da)
    .find((a) => a.place?.lat && a.place?.lng);
...
const DayPlanSidebar = React.memo(function DayPlanSidebar(props: DayPlanSidebarProps) {
  ...
  return (
    ...
    >
      ...
      {/* Tagesliste */}
      <div
        className={`scroll-container${draggingId ? '' : 'odyssey-stagger'}`}
          ...
          return (
            <div key={day.id} style={{ borderBottom: '1px solid var(--border-faint)' }}>
              ...
              <div
                ...
                onDragOver={(e) => {
                  e.preventDefault();
                  if (dragOverDayId !== day.id) setDragOverDayId(day.id);
                }}
                onDragLeave={(e) => {
                  if (!e.currentTarget.contains(e.relatedTarget as Node | null)) setDragOverDayId(null);
                }}
                ...
                onMouseEnter={(e) => {
                  if (!isSelected && !isDragTarget) e.currentTarget.style.background = 'var(--bg-tertiary)';
                }}
                onMouseLeave={(e) => {
                  if (!isSelected)
                    e.currentTarget.style.background = isDragTarget ? 'rgba(17,24,39,0.07)' : 'transparent';
                }}
              >
                ...
                {(() => {
                  ...
                  const geoLat = anyGeoPlace
                    ? 'place' in anyGeoPlace
                      ? anyGeoPlace.place?.lat
                      : anyGeoPlace.lat
                    : undefined;
                  const geoLng = anyGeoPlace
                    ? 'place' in anyGeoPlace
                      ? anyGeoPlace.place?.lng
                      : anyGeoPlace.lng
                    : undefined;
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }}>
                        ...
                        {formattedDate && (
                          <>
                            <span
                              style={{ flexShrink: 0, width: 1, height: 11, background: 'var(--border-primary)' }}
                            />
                            <span
                              style={{
                                flexShrink: 0,
                      ...
                      {(() => {
                        const hasAccs = accommodations.some((a) =>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'nowrap', minWidth: 0 }}>
                        {(() => {
                          ...
                          return dayAccs.map((acc) => {
                            ...
                            const iconColor =
                              isCheckOut && !isCheckIn ? '#ef4444' : isCheckIn ? '#22c55e' : 'var(--text-faint)';
                            return (
                              <span
                                key={acc.id}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if ((acc as any).place_id) onPlaceClick((acc as any).place_id);
                                }}
                                ...
                                style={{
                                  ...
                                  flexShrink: 1,
                                  minWidth: 0,
                                  cursor: (acc as any).place_id ? 'pointer' : 'default',
                                  borderRadius: 7,
                  ...
                  {cost && (
                    <div style={{ marginTop: 2 }}>
                      <span className="text-[#059669]" style={{ fontSize: 11 }}>
                        {cost}
                      </span>
                    </div>
                  )}
                    ...
                    return (
                      ...
                      >
                        <button
                          ...
                          style={{ ...cell, border: 'none', borderRight: div, borderBottom: div }}
                        >
                        ...
                        {onAddTransport ? (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              onAddTransport(day.id);
                            }}
                            title={t('transport.addTransport')}
                            style={{ ...cell, border: 'none', borderBottom: div }}
                          >
                        ...
                        <button
                          ...
                          style={{ ...cell, border: 'none', borderRight: div }}
                        >
              ...
              {isExpanded && (
                <div
                  ...
                  onDragOver={(e) => {
                    ...
                    if (draggingId && (!cur || cur.startsWith('end-'))) setDropTargetKey(`end-${day.id}`);
                  }}
                  onDrop={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    ...
                    if (dropTargetRef.current?.startsWith('transport-')) {
                      ...
                      const parts = dropTargetRef.current
                        .replace('transport-after-', '')
                        .replace('transport-', '')
                        .split('-');
                      ...
                      if (placeId) {
                        ...
                        const r = reservations.find((x) => x.id === Number(fromReservationId));
                        if (r) {
                          const update = computeMultiDayMove(r, day.id, phase);
                          tripActions
                            .updateReservation(tripId, r.id, update)
                            .catch((err: unknown) =>
                              toast.error(err instanceof Error ? err.message : t('common.unknownError'))
                        ...
                        tripActions
                          .moveAssignment(tripId, Number(assignmentId), fromDayId, day.id)
                          .catch((err: unknown) =>
                            toast.error(err instanceof Error ? err.message : t('common.unknownError'))
                          );
                        ...
                        tripActions
                          .moveDayNote(tripId, fromDayId, day.id, Number(noteId))
                          .catch((err: unknown) =>
                            toast.error(err instanceof Error ? err.message : t('common.unknownError'))
                          );
                      } else if (noteId) {
                    ...
                    if (!assignmentId && !noteId && !placeId) {
                      dragDataRef.current = null;
                      window.__dragData = null;
                      return;
                    }
                ...
                >
                  ...
                  {merged.length === 0 && !dayNoteUi ? (
                    ...
                    merged.map((item, idx) => {
                      ...
                      const itemKey =
                        item.type === 'transport'
                          ? `transport-${item.data.id}${legSuffix}-${day.id}`
                          : item.type === 'place'
                            ? `place-${item.data.id}`
                            : `note-${item.data.id}`;
                      ...
                      const showDropLineAfter =
                        ...
                        dropTargetKey === `transport-after-${item.data.id}${legSuffix}-${day.id}`;
                      ...
                      if (item.type === 'place') {
                        ...
                        const isPlaceSelected = selectedAssignmentId
                        ...
                        const arrowMove = (direction: 'up' | 'down') => {
                          ...
                          if (parseTimeToMinutes(placeTime) !== null) {
                            const timedInNewOrder = newOrder
                              ...
                              .filter((t) => t !== null);
                            const isChronological = timedInNewOrder.every(
                              (t, i) => i === 0 || t >= timedInNewOrder[i - 1]
                            );
                            if (!isChronological) {
                              const timeStr = placeTime.includes(':') ? placeTime.substring(0, 5) : placeTime;
                              ...
                              setTimeConfirm({
                                ...
                                time: timeStr,
                                reorderIds: newOrder.filter((i) => i.type === 'place').map((i) => i.data.id),
                              });
                              return;
                            }
                          }
                          ...
                          <React.Fragment key={`place-${assignment.id}`}>
                            <div
                              ...
                              onDragOver={(e) => {
                                ...
                                e.stopPropagation();
                                setDragOverDayId(null);
                                if (dropTargetKey !== `place-${assignment.id}`)
                                  setDropTargetKey(`place-${assignment.id}`);
                              }}
                              onDrop={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                ...
                                if (placeId) {
                                  ...
                                  tripActions
                                    .moveAssignment(tripId, Number(fromAssignmentId), fromDayId, day.id, toIdx)
                                    .catch((err: unknown) =>
                                      toast.error(err instanceof Error ? err.message : t('common.unknownError'))
                                    );
                                  setDraggingId(null);
                                  ...
                                  const so =
                                    toIdx <= 0
                                      ? (tm[0]?.sortKey ?? 0) - 1
                                      : (tm[toIdx - 1].sortKey + tm[toIdx].sortKey) / 2;
                                  tripActions
                                    .moveDayNote(tripId, fromDayId, day.id, Number(noteId), so)
                                    .catch((err: unknown) =>
                                      toast.error(err instanceof Error ? err.message : t('common.unknownError'))
                                    );
                                  setDraggingId(null);
                              ...
                              onClick={() => {
                                onPlaceClick(isPlaceSelected ? null : place.id, isPlaceSelected ? null : assignment.id);
                                if (!isPlaceSelected) onSelectDay(day.id, true);
                              }}
                              onContextMenu={(e) =>
                                ctxMenu.open(e, [
                                  canEditDays &&
                                  ...
                                  canEditDays &&
                                    onRemoveAssignment && {
                                      label: t('planner.removeFromDay'),
                                      icon: Trash2,
                                      onClick: () => onRemoveAssignment(day.id, assignment.id),
                                    },
                                  place.website && {
                                  ...
                                  place.lat &&
                                    place.lng && {
                                      ...
                                      onClick: () =>
                                        window.open(
                                          `https://www.google.com/maps/search/?api=1&query=${place.google_place_id ? encodeURIComponent(place.name) + '&query_place_id=' + place.google_place_id : place.lat + ',' + place.lng}`,
                                          '_blank'
                                        ),
                                    },
                                  { divider: true },
                            ...
                            >
                              ...
                              <div
                                onClick={(e) => {
                                  e.stopPropagation();
                                  toggleLock(assignment.id);
                                }}
                                onMouseEnter={(e) => {
                                  e.stopPropagation();
                                  setLockHoverId(assignment.id);
                                }}
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 4, overflow: 'hidden' }}>
                                  ...
                                  {place.place_time && (
                                    ...
                                    >
                                      ...
                                      {place.end_time ? ` – ${formatTime(place.end_time, locale, timeFormat)}` : ''}
                                    </span>
                                  )}
                                </div>
                                {(place.description || place.address || cat?.name) && (
                                  <div
                                    className="collab-note-md"
                                    style={{
                                      marginTop: 2,
                                      fontSize: 10,
                                      color: 'var(--text-faint)',
                                        ...
                                        {(() => {
                                          const meta =
                                            typeof res.metadata === 'string'
                                              ...
                                              : res.metadata || {};
                                          if (!meta) return null;
                                          if (meta.airline && meta.flight_number)
                                            return (
                                              <span style={{ fontWeight: 400 }}>
                                          ...
                                          if (meta.flight_number)
                                            return <span style={{ fontWeight: 400 }}>{meta.flight_number}</span>;
                                          if (meta.train_number)
                                            return <span style={{ fontWeight: 400 }}>{meta.train_number}</span>;
                                          return null;
                                        })()}
                                      </div>
                                      {hasEndpoints && (
                                ...
                                {assignment.participants?.length > 0 && (
                                  <div style={{ marginTop: 3, display: 'flex', alignItems: 'center', gap: -4 }}>
                                    ...
                                    {assignment.participants.length > 5 && (
                                      <span className="text-content-faint" style={{ fontSize: 8, marginLeft: 2 }}>
                                        +{assignment.participants.length - 5}
                                      </span>
                                    )}
                                  </div>
                              ...
                              {canEditDays && (
                                <div
                                  ...
                                  style={{ flexShrink: 0, display: 'flex', gap: 1, transition: 'opacity 0.15s' }}
                                >
                                  <button
                                    onClick={moveUp}
                                    disabled={idx === 0}
                                    className={idx === 0 ? 'text-[var(--border-primary)]' : 'text-content-faint'}
                                    style={{
                                      background: 'none',
                                      border: 'none',
                                      padding: '1px 2px',
                                  ...
                                  <button
                                    ...
                                    className={
                                      idx === merged.length - 1 ? 'text-[var(--border-primary)]' : 'text-content-faint'
                                    }
                                    style={{
                                      background: 'none',
                                      border: 'none',
                                      padding: '1px 2px',
                                      cursor: idx === merged.length - 1 ? 'default' : 'pointer',
                                      display: 'flex',
                                      lineHeight: 1,
                                    }}
                                  >
                          ...
                          <React.Fragment key={`transport-${res.id}-${legKey}-${day.id}`}>
                            <div
                              ...
                              onDragOver={(e) => {
                                ...
                                const key = inBottom
                                  ? `transport-after-${res.id}${ls}-${day.id}`
                                  : `transport-${res.id}${ls}-${day.id}`;
                                if (dropTargetRef.current !== key) setDropTargetKey(key);
                              }}
                              draggable={canEditDays && spanPhase !== 'middle' && !res.__leg}
                              onDragStart={(e) => {
                                if (!canEditDays || spanPhase === 'middle' || res.__leg) {
                                  e.preventDefault();
                                  return;
                                }
                              ...
                              onDrop={(e) => {
                                ...
                                if (placeId) {
                                  ...
                                  if (r2) {
                                    const update = computeMultiDayMove(r2, day.id, phase);
                                    tripActions
                                      .updateReservation(tripId, r2.id, update)
                                      .catch((err: unknown) =>
                                        toast.error(err instanceof Error ? err.message : t('common.unknownError'))
                                      );
                                  }
                                  ...
                                  handleMergedDrop(
                                    ...
                                    res.id,
                                    insertAfter,
                                    res.__leg?.index ?? null
                                  );
                                } else if (fromAssignmentId && fromDayId !== day.id) {
                                  tripActions
                                    .moveAssignment(tripId, Number(fromAssignmentId), fromDayId, day.id)
                                    .catch((err: unknown) =>
                                      toast.error(err instanceof Error ? err.message : t('common.unknownError'))
                                    );
                                } else if (fromAssignmentId) {
                              ...
                              style={{
                                ...
                                background: `${color}08`,
                                cursor: canEditDays && onEditTransport ? 'pointer' : 'default',
                                userSelect: 'none',
                                  ...
                                  {(() => {
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        {spanPhase === 'single' && endTime
                                          ? ` – ${formatTime(endTime, locale, timeFormat)}`
                                          : ''}
                                        {meta.departure_timezone &&
                                          spanPhase === 'start' &&
                                          ` ${meta.departure_timezone}`}
                        ...
                        <React.Fragment key={`note-${note.id}`}>
                          <div
                            ...
                            onDragStart={(e) => {
                              ...
                              e.dataTransfer.setData('fromDayId', String(day.id));
                              e.dataTransfer.effectAllowed = 'move';
                              dragDataRef.current = { noteId: String(note.id), fromDayId: String(day.id) };
                            ...
                            onDragOver={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              if (dropTargetKey !== `note-${note.id}`) setDropTargetKey(`note-${note.id}`);
                            }}
                            onDrop={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              ...
                              if (placeId) {
                                ...
                                const so =
                                  toIdx <= 0
                                    ...
                                    : (tm[toIdx - 1].sortKey + tm[toIdx].sortKey) / 2;
                                tripActions
                                  .moveDayNote(tripId, fromDayId, day.id, Number(fromNoteId), so)
                                  .catch((err: unknown) =>
                                    toast.error(err instanceof Error ? err.message : t('common.unknownError'))
                                  );
                                setDraggingId(null);
                                    ...
                                    ctxMenu.open(e, [
                                      {
                                        label: t('common.edit'),
                                        icon: Pencil,
                                        onClick: () => openEditNote(day.id, note),
                                      },
                                      { divider: true },
                                      {
                                        label: t('common.delete'),
                                        icon: Trash2,
                                        danger: true,
                                        onClick: () => setPendingDeleteNote({ dayId: day.id, noteId: note.id }),
                                      },
                                    ])
                                : undefined
                            }
                            onMouseEnter={(e) => {
                              const grip = e.currentTarget.querySelector('.dp-grip') as HTMLElement | null;
                              if (grip) grip.style.opacity = '1';
                              const editBtns = e.currentTarget.querySelector(
                                '.note-edit-buttons'
                              ) as HTMLElement | null;
                          ...
                          >
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              {note.time && (
                                ...
                                >
                                  <Markdown remarkPlugins={[remarkGfm]}>{note.time}</Markdown>
                                </div>
                              )}
                            </div>
                            {canEditDays && (
                              <div
                                className="note-edit-buttons"
                                style={{
                                  display: 'flex',
                                  gap: 1,
                                  flexShrink: 0,
                              ...
                              >
                                <button
                                  onClick={(e) => openEditNote(day.id, note, e)}
                                  className="text-content-faint"
                                  style={{
                                    background: 'none',
                                    border: 'none',
                                ...
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setPendingDeleteNote({ dayId: day.id, noteId: note.id });
                                  }}
                                  className="text-content-faint"
                                  style={{
                                    background: 'none',
                            ...
                            {canEditDays && (
                              ...
                              >
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    moveNote(day.id, note.id, 'up');
                                  }}
                                  disabled={noteIdx === 0}
                                  className={noteIdx === 0 ? 'text-[var(--border-primary)]' : 'text-content-faint'}
                                  style={{
                                    background: 'none',
                                    border: 'none',
                                    padding: '1px 2px',
                                ...
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    moveNote(day.id, note.id, 'down');
                                  }}
                                  disabled={noteIdx === merged.length - 1}
                                  className={
                                    noteIdx === merged.length - 1
                                  ...
                                  style={{
                                    ...
                                    border: 'none',
                                    padding: '1px 2px',
                                    cursor: noteIdx === merged.length - 1 ? 'default' : 'pointer',
                                    display: 'flex',
                                    lineHeight: 1,
                                  }}
                                >
                  ...
                  <div
                    ...
                    onDragOver={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      if (dropTargetKey !== `end-${day.id}`) setDropTargetKey(`end-${day.id}`);
                    }}
                    onDrop={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      ...
                      if (!assignmentId && !noteId && !fromReservationId) {
                        dragDataRef.current = null;
                        window.__dragData = null;
                        return;
                      }
                  ...
                  {(isSelected || (showRouteToolsWhenExpanded && isExpanded)) &&
                    getDayAssignments(day.id).length >= 2 && (
                      <div
                        style={{
                            ...
                            {(['driving', 'walking'] as const).map((p) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={
                                    active ? 'bg-accent text-accent-text' : 'bg-transparent text-content-secondary'
                                  }
                                  style={{
                                    display: 'flex',
                                    alignItems: 'center',
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (pendingDeleteNote) deleteNote(pendingDeleteNote.dayId, pendingDeleteNote.noteId);
        }}
