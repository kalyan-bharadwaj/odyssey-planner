...
function buildLocationEl(): { root: HTMLDivElement; cone: HTMLDivElement } {
  ...
  pulse.style.cssText = `
    ...
    animation:odyssey-location-pulse 2s ease-out infinite;
  `
...
function ensurePulseStyle() {
  if (document.getElementById('odyssey-location-style')) return
  const s = document.createElement('style')
  s.id = 'odyssey-location-style'
  s.textContent = `
    @keyframes odyssey-location-pulse {
...
export function attachLocationMarker(map: mapboxgl.Map): LocationMarkerHandle {
  ...
  const ensureAccuracyLayer = () => {
    if (map.getSource('odyssey-location-accuracy')) return
    try {
      map.addSource('odyssey-location-accuracy', {
        type: 'geojson',
      ...
      map.addLayer({
        id: 'odyssey-location-accuracy',
        type: 'fill',
        source: 'odyssey-location-accuracy',
        paint: {
          'fill-color': '#3b82f6',
          'fill-opacity': 0.14,
  ...
  const setAccuracy = (p: GeoPosition) => {
    const src = map.getSource('odyssey-location-accuracy') as mapboxgl.GeoJSONSource | undefined
    if (!src) return
  ...
  const handle: LocationMarkerHandle = {
    ...
    destroy: () => {
      ...
      try {
        if (map.getLayer('odyssey-location-accuracy')) map.removeLayer('odyssey-location-accuracy')
        if (map.getSource('odyssey-location-accuracy')) map.removeSource('odyssey-location-accuracy')
      } catch { /* noop */ }
    },
  }
