import { escapeHtml } from '@odyssey/shared';
import L from 'leaflet';
import { Bike, Bus, Car, CarTaxiFront, Plane, Route, Sailboat, Ship, Train } from 'lucide-react';
...
function endpointIcon(type: TransportType, label: string | null): L.DivIcon {
  ...
  return L.divIcon({
    className: 'odyssey-endpoint-marker',
    html: `<div style="
...
function buildStatsHtml(
  ...
  const main = mainLabel
    ? `<span style="font-size:12px;font-weight:700;line-height:1;display:block">${escapeHtml(mainLabel)}</span>`
    : '';
  const sub = subLabel
    ? `<span style="font-size:10px;font-weight:500;line-height:1;opacity:0.85;display:block${hasBoth ? ';margin-top:4px' : ''}">${escapeHtml(subLabel)}</span>`
    : '';
  const html = `<div class="odyssey-stats-inner" style="
...
function StatsLabel({ item }: { item: TransportItem }) {
  ...
  const apply = () => {
    ...
    if (!innerRef.current && el) innerRef.current = el.querySelector('.odyssey-stats-inner') as HTMLElement | null;
    if (innerRef.current) innerRef.current.style.transform = `rotate(${pose.angle}deg)`;
  };

  useEffect(() => {
    const icon = L.divIcon({
      className: 'odyssey-endpoint-stats',
      html,
      ...
      {visibleItems.flatMap((item) =>
        item.waypoints.map((wp, wi) => (
          <Marker
            ...
            icon={endpointIcon(
              item.type,
              showEndpointLabels && labelVisibleIds.has(item.res.id) ? wp.code || cleanName(wp.name) : null
            )}
            pane={ENDPOINT_PANE}
            zIndexOffset={1000}
