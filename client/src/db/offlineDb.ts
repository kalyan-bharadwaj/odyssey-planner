 ...
 * the base name; a logged-in user uses `odyssey-offline-u<userId>`.
 */
const ANON_DB_NAME = 'odyssey-offline';

function userDbName(userId: number | string): string {
  return `odyssey-offline-u${userId}`;
}
...
function initialDbName(): string {
  try {
    const raw = typeof localStorage !== 'undefined' ? localStorage.getItem('odyssey_auth_snapshot') : null;
...
class OdysseyOfflineDb extends Dexie {
...
let _db = new OdysseyOfflineDb(initialDbName());

export const offlineDb = new Proxy({} as OdysseyOfflineDb, {
  get(_target, prop) {
...
export async function deleteCurrentUserDb(): Promise<void> {
  ...
  _db = new OdysseyOfflineDb(ANON_DB_NAME);
  await _db.open();
}
