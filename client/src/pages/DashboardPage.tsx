...
export default function DashboardPage(): React.ReactElement {
  ...
  return (
    <>
      {/* Navbar lives outside .odyssey-dash so it keeps the app-wide font + button
          styling instead of inheriting the dashboard scope's font and the
          `.odyssey-dash button` reset (which shifted the bell icon + menu items). */}
      <Navbar />
      <div className="odyssey-dash odyssey-dash-shell">
        {demoMode && <DemoBanner />}
        <div className="odyssey-dash-scroll">
          <MobileTopBar />
          <main className="page">
    ...
    <section className="atlas">
      ...
      <div className="atlas-card">
        ...
        <svg className="spark" width="80" height="36" viewBox="0 0 80 36">
          <polyline
            points="0,30 12,26 22,28 32,18 44,22 56,10 68,14 80,4"
            fill="none"
            strokeWidth="2"
            strokeLinecap="round"
      ...
      <div className="atlas-card">
        ...
        <div className="value mono">
          {stats?.totalDays ?? 0} <span className="unit">{t('dashboard.atlas.daysUnit')}</span>
        </div>
      ...
      <div className="atlas-card">
        ...
        <div className="value mono">
          {distanceText} <span className="unit">{t('dashboard.atlas.kmUnit')}</span>
        </div>
...
function CurrencyTool(): React.ReactElement {
  const { t } = useTranslation();
  const [from, setFrom] = useState(() => localStorage.getItem('odyssey_fx_from') || 'EUR');
  const [to, setTo] = useState(() => localStorage.getItem('odyssey_fx_to') || 'USD');
  ...
  useEffect(() => {
    localStorage.setItem('odyssey_fx_from', from);
    localStorage.setItem('odyssey_fx_to', to);
  }, [from, to]);
    ...
    <div className="tool">
      <div className="tool-head">
        <div className="tool-title">
          <RefreshCw size={14} /> {t('dashboard.currency')}
        </div>
        <button className="tool-action" aria-label={t('dashboard.aria.refreshRates')} onClick={fetchRate}>
          <RefreshCw size={14} />
        </button>
      </div>
      <div className="fx-input">
        ...
        <button className="fx-swap" aria-label={t('dashboard.aria.swapCurrencies')} onClick={swap}>
...
function TimezoneTool({ locale }: { locale: string }): React.ReactElement {
  ...
  const [zones, setZones] = useState<string[]>(() => {
    try {
      const raw = localStorage.getItem('odyssey_dashboard_tz');
  ...
  useEffect(() => {
    localStorage.setItem('odyssey_dashboard_tz', JSON.stringify(zones));
  }, [zones]);
  ...
  const addZone = (tz: string) => {
    if (tz) setZones((prev) => (prev.includes(tz) ? prev : [...prev, tz]));
    setAdding(false);
  };
  ...
  return (
    <div className="tool">
      <div className="tool-head">
        <div className="tool-title">
          <Clock size={14} /> {t('dashboard.timezone')}
        </div>
        <button
          className="tool-action"
      ...
      {adding && (
        <div style={{ marginBottom: 14 }}>
          <CustomSelect
            ...
            searchable
            size="sm"
            placeholder={t('dashboard.tz.searchPlaceholder')}
          />
        </div>
      )}
