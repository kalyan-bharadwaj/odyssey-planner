import { continentForCountry } from '@odyssey/shared';
import { Briefcase, Globe, MapPin, Plus, Search, Star, X } from 'lucide-react';
...
function MobileStats({
  ...
  const CL = {
    Europe: t('atlas.europe'),
    Asia: t('atlas.asia'),
    'North America': t('atlas.northAmerica'),
    'South America': t('atlas.southAmerica'),
    Africa: t('atlas.africa'),
    Oceania: t('atlas.oceania'),
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-5 gap-2">
        {[
          ...
          <div key={i} className="py-2 text-center">
            <p className="text-xl font-black tabular-nums" style={{ color: tp }}>
              {v}
            </p>
            <p className="text-[9px] font-semibold uppercase tracking-wide" style={{ color: tf }}>
              {l}
            </p>
          </div>
        ))}
      ...
      <div className="grid grid-cols-6 gap-1">
        {['Europe', 'Asia', 'North America', 'South America', 'Africa', 'Oceania'].map((cont) => {
          ...
          return (
            <div key={cont} className="py-1 text-center">
              <p
                className="text-base font-bold tabular-nums"
                style={{ color: count > 0 ? tp : dark ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.12)' }}
              >
                {count}
              </p>
              <p
                className="text-[8px] font-semibold uppercase"
                style={{ color: count > 0 ? tf : dark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)' }}
              >
                {CL[cont]}
              </p>
            </div>
    ...
    return (
      <div className="min-h-screen bg-surface">
        ...
        <div
          ...
          style={{ paddingTop: 'var(--nav-h)', minHeight: 'calc(100vh - var(--nav-h))' }}
        >
  ...
  return (
    <div className="h-screen overflow-hidden bg-surface">
      ...
      >
        ...
        {/* Mobile: Bottom bar */}
        <div
          className="absolute left-0 right-0 z-10 flex justify-center md:hidden"
          style={{ bottom: 'calc(84px + env(safe-area-inset-bottom, 0px) + 8px)', touchAction: 'manipulation' }}
        >
          <div
          ...
          >
            ...
            >
              ...
              <p className="mt-1 text-[9px] font-semibold uppercase tracking-wide text-content-faint">
                {t('atlas.countries')}
              </p>
            </div>
            {[
              ...
              <div key={i} className="px-1 text-center">
                ...
                <p className="mt-1 text-[9px] font-semibold uppercase tracking-wide text-content-faint">{l}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div
          ...
          className="absolute z-10 hidden flex-col overflow-hidden transition-[width,height,transform,box-shadow] duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] md:flex"
          style={{
            bottom: 16,
            left: '50%',
        ...
        >
          ...
          <SidebarContent
            ...
            onCountryClick={loadCountryDetail}
            onTripClick={(id) => navigate(`/trips/${id}`)}
            onUnmarkCountry={handleUnmarkCountry}
            bucketList={bucketList}
            bucketTab={bucketTab}
            setBucketTab={setBucketTab}
            showBucketAdd={showBucketAdd}
            setShowBucketAdd={setShowBucketAdd}
            bucketForm={bucketForm}
            setBucketForm={setBucketForm}
            onAddBucket={handleAddBucketItem}
            onDeleteBucket={handleDeleteBucketItem}
            onSearchBucket={handleBucketPoiSearch}
            onSelectBucketPoi={handleSelectBucketPoi}
            bucketSearchResults={bucketSearchResults}
            setBucketSearchResults={setBucketSearchResults}
            bucketPoiMonth={bucketPoiMonth}
            setBucketPoiMonth={setBucketPoiMonth}
            bucketPoiYear={bucketPoiYear}
            setBucketPoiYear={setBucketPoiYear}
            bucketSearching={bucketSearching}
        ...
        >
          ...
          >
            {confirmAction.code.length === 2 ? (
              <img
                src={`https://flagcdn.com/w80/${confirmAction.code.toLowerCase()}.png`}
                alt={confirmAction.code}
                style={{
                  width: 48,
                  height: 34,
                  borderRadius: 6,
            ...
            <h3 className="text-content" style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 700 }}>
              {confirmAction.name}
            </h3>
            ...
            {confirmAction.type === 'choose' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                >
                  ...
                  <div>
                    <div className="text-content" style={{ fontSize: 13, fontWeight: 600 }}>
                      {t('atlas.markVisited')}
                    </div>
                    <div className="text-content-muted" style={{ fontSize: 11, marginTop: 1 }}>
                      {t('atlas.markVisitedHint')}
                    </div>
                  </div>
                </button>
                <button
                  onClick={() => setConfirmAction({ ...confirmAction, type: 'bucket' as any })}
                  className="border border-edge"
                  style={{
                    display: 'flex',
                    alignItems: 'center',
            ...
            {confirmAction.type === 'choose-region' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {confirmAction.countryName && (
                  <p className="text-content-muted" style={{ margin: '-8px 0 8px', fontSize: 12 }}>
                    {confirmAction.countryName}
                  </p>
                )}
                <button
                  onClick={async () => {
                    ...
                    if (!rCode) return;
                    try {
                      await apiClient.post(`/addons/atlas/region/${rCode}/mark`, {
                        name: rName,
                      ...
                      setVisitedRegions((prev) => {
                        ...
                        return {
                          ...
                          [countryCode]: [
                            ...existing,
                            { code: rCode, name: rName, placeCount: 0, manuallyMarked: true },
                          ],
                        };
                      });
                      setData((prev) => {
                        ...
                        return {
                          ...
                          countries: [
                            ...prev.countries,
                            { code: countryCode, placeCount: 0, tripCount: 0, firstVisit: null, lastVisit: null },
                          ],
                          stats: { ...prev.stats, totalCountries: prev.stats.totalCountries + 1 },
                          continents: { ...prev.continents, [cont]: (prev.continents?.[cont] || 0) + 1 },
                        };
            ...
            {confirmAction.type === 'unmark' && (
              <>
                <p className="text-content-muted" style={{ margin: '0 0 20px', fontSize: 13 }}>
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setData((prev) => {
                          ...
                          const remainingRegions = (visitedRegions[countryCode] || []).filter(
                            (r) => r.code !== rCode && r.manuallyMarked
                          );
            ...
            {confirmAction.type === 'bucket' && (
              <>
                <p className="text-content-muted" style={{ margin: '0 0 14px', fontSize: 13 }}>
                  {t('atlas.bucketWhen')}
                </p>
                    ...
                    <CustomSelect
                      ...
                      options={[
                        ...
                        ...Array.from({ length: 12 }, (_, i) => ({
                          value: String(i + 1),
                          label: new Date(2000, i).toLocaleString(language, { month: 'long' }),
                        })),
                      ]}
                      size="sm"
                    />
                ...
                <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
                  <button
                    onClick={() =>
                      setConfirmAction({
                        ...confirmAction,
                        type: confirmAction.regionCode ? 'choose-region' : 'choose',
                      })
                    }
                    className="border border-edge text-content-muted"
                    style={{
...
function SidebarContent({
  ...
  const tabBar = (
    <div style={{ display: 'flex', gap: 4, padding: '12px 16px 0', marginBottom: 4 }}>
      {[
        { id: 'stats', label: t('atlas.statsTab'), icon: Globe },
        { id: 'bucket', label: t('atlas.bucketTab'), icon: Star },
      ].map((tab) => (
        <button
          key={tab.id}
          onClick={() => setBucketTab(tab.id as any)}
          style={{
            flex: 1,
            display: 'flex',
            alignItems: 'center',
    ...
    return (
      <>
        ...
        <div className="p-8 text-center">
          ...
          <p className="text-sm font-medium" style={{ color: tm }}>
            {t('atlas.noData')}
          </p>
          <p className="mt-1 text-xs" style={{ color: tf }}>
            {t('atlas.noDataHint')}
          </p>
        </div>
      </>
  ...
  const bucketContent = (
    <>
      <div
        ...
        style={{ overflowX: 'auto', padding: '0 8px', maxWidth: statsWidth, width: '100%' }}
      >
        {bucketList.map((item) => (
          <div
            key={item.id}
            className="group flex shrink-0 flex-col items-center justify-center"
            style={{ padding: '8px 14px', position: 'relative', minWidth: 80 }}
          >
            {(() => {
              const code =
                item.country_code?.length === 2
                  ? item.country_code
                  : Object.entries(A2_TO_A3).find(([, v]) => v === item.country_code)?.[0] || '';
              return code ? (
                <img
                  src={`https://flagcdn.com/w40/${code.toLowerCase()}.png`}
                  alt={code}
                  style={{ width: 28, height: 20, borderRadius: 4, objectFit: 'cover', marginBottom: 4 }}
                />
              ) : (
            ...
            <span
              ...
              style={{ color: tp, maxWidth: 90, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
            >
              {item.name}
            </span>
            {item.target_date &&
              (() => {
                ...
                const label = m
                  ? new Date(Number(y), Number(m) - 1).toLocaleString(language, { month: 'short', year: 'numeric' })
                  : y;
                return (
                  <span className="mt-0.5 text-center text-[9px]" style={{ color: tf }}>
                    {label}
                  </span>
                );
              })()}
            {!item.target_date && item.notes && (
              <span
                className="mt-0.5 text-center text-[9px]"
                style={{ color: tf, maxWidth: 90, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
              >
                {item.notes}
              </span>
            )}
        ...
        {bucketList.length === 0 && !showBucketAdd && (
          <div className="flex items-center justify-center px-6 py-4" style={{ color: tf, fontSize: 12 }}>
      ...
      {showBucketAdd ? (
        <div style={{ padding: '8px 16px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <div style={{ position: 'relative' }}>
            <div style={{ display: 'flex', gap: 4 }}>
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !bucketForm.name) onSearchBucket();
                  else if (e.key === 'Enter') onAddBucket();
                  if (e.key === 'Escape') setShowBucketAdd(false);
                }}
                placeholder={t('atlas.bucketNamePlaceholder')}
                autoFocus
                className="border border-edge bg-surface-input text-content"
                style={{
                  flex: 1,
                  padding: '6px 10px',
                  borderRadius: 8,
            ...
            {bucketSearchResults.length > 0 && (
              ...
              >
                {bucketSearchResults.slice(0, 6).map((r, i) => (
                  <button
                    key={i}
                    onClick={() => onSelectBucketPoi(r)}
                    className="border-b border-edge-faint"
                    style={{
                      display: 'flex',
                  ...
                  >
                    <span className="text-content" style={{ fontSize: 12, fontWeight: 500 }}>
                      {r.name}
                    </span>
                    {r.address && (
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <div style={{ flex: 1 }}>
              <CustomSelect
                value={String(bucketPoiMonth)}
                onChange={(v) => setBucketPoiMonth(Number(v))}
                placeholder={t('atlas.month')}
                size="sm"
                options={[
                  { value: '0', label: '—' },
                  ...Array.from({ length: 12 }, (_, i) => ({
                    value: String(i + 1),
                    label: new Date(2000, i).toLocaleString(language, { month: 'short' }),
                  })),
                ]}
              />
            </div>
            <div style={{ flex: 1 }}>
              <CustomSelect
                value={String(bucketPoiYear)}
                onChange={(v) => setBucketPoiYear(Number(v))}
                placeholder={t('atlas.year')}
                size="sm"
                options={[
                  { value: '0', label: '—' },
          ...
          <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                fontFamily: 'inherit',
                opacity: bucketForm.name.trim() ? 1 : 0.5,
              }}
            >
              {t('common.add')}
            </button>
      ...
      <div style={{ display: 'grid' }}>
        <div style={bucketTab === 'bucket' ? { visibility: 'hidden' as const, gridArea: '1/1' } : { gridArea: '1/1' }}>
          <div ref={statsContentRef} className="flex items-stretch justify-center">
            ...
            {/* Countries hero */}
            <div
              className="mx-2 my-2 flex items-baseline gap-1.5 rounded-xl px-5 py-4"
              style={{ background: bg(0.08) }}
            >
              <span className="text-5xl font-black tabular-nums leading-none" style={{ color: tp }}>
                {stats.totalCountries}
              </span>
              <span className="text-sm font-medium" style={{ color: tm }}>
                {t('atlas.countries')}
              </span>
            </div>
            {/* Other stats */}
            {[
              ...
              <div key={i} className="flex shrink-0 flex-col items-center justify-center px-3 py-5">
                <span className="text-2xl font-black tabular-nums leading-none" style={{ color: tp }}>
                  {v}
                </span>
                <span
                  className="mt-1.5 whitespace-nowrap text-[9px] font-semibold uppercase tracking-wide"
                  style={{ color: tf }}
                >
                  {l}
                </span>
            ...
            <div className="flex shrink-0 items-center gap-4 px-3 py-4">
              {['Europe', 'Asia', 'North America', 'South America', 'Africa', 'Oceania'].map((cont) => {
                const count = continents?.[cont] || 0;
                const active = count > 0;
                return (
                  <div key={cont} className="flex shrink-0 flex-col items-center">
                    <span
                      className="text-2xl font-black tabular-nums leading-none"
                      style={{ color: active ? tp : bg(0.15) }}
                    >
                      {count}
                    </span>
                    <span
                      className="mt-1.5 whitespace-nowrap text-[9px] font-semibold uppercase tracking-wide"
                      style={{ color: active ? tf : bg(0.1) }}
                    >
                      {CL[cont]}
                    </span>
                  </div>
            ...
            <div className="flex items-center gap-5 px-3 py-4">
              ...
              {lastTrip && (
                <button
                  onClick={() => onTripClick(lastTrip.id)}
                  className="flex items-center gap-2.5 text-left transition-opacity hover:opacity-75"
                >
                  <div
                    className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-lg"
                    style={{ background: bg(0.06) }}
                  >
                    {lastTrip.countryCode ? (
                      countryCodeToFlag(lastTrip.countryCode)
                    ) : (
                      <MapPin size={16} style={{ color: tm }} />
                    )}
                  </div>
                  <div className="min-w-0">
                    <p className="text-[9px] font-semibold uppercase tracking-wider" style={{ color: tf }}>
                      {t('atlas.lastTrip')}
                    </p>
                    <p className="truncate text-[13px] font-bold" style={{ color: tp }}>
                      {lastTrip.title}
                    </p>
                  </div>
                </button>
              ...
              {streak > 0 && (
                <div className="flex flex-col items-center justify-center px-3">
                  ...
                  <span
                    className="mt-1.5 whitespace-nowrap text-center text-[9px] font-semibold uppercase leading-tight tracking-wide"
                    style={{ color: tf }}
                  >
                    {streak === 1 ? t('atlas.yearInRow') : t('atlas.yearsInRow')}
                  </span>
              ...
              <>
                ...
                <div className="flex items-center gap-3 px-6 py-4">
                  ...
                  <div>
                    ...
                    <p className="mb-1 text-[10px]" style={{ color: tf }}>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {countryDetail.trips.slice(0, 3).map((trip) => (
                        <button
                          ...
                          className="flex items-center gap-1 rounded px-2 py-0.5 text-[10px] font-semibold transition-opacity hover:opacity-75"
                      ...
                      {countryDetail.manually_marked && onUnmarkCountry && (
                        <button
                          onClick={() => onUnmarkCountry(selectedCountry!)}
                          className="flex items-center gap-1 rounded bg-[rgba(239,68,68,0.1)] px-2 py-0.5 text-[10px] font-semibold text-[#ef4444] transition-opacity hover:opacity-75"
                        >
                          <X size={9} />
                          {t('atlas.unmark')}
                        </button>
