...
export function useDashboard() {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>(() => (localStorage.getItem('odyssey_dashboard_view') as 'grid' | 'list') || 'grid')
  ...
  const toggleViewMode = () => {
    setViewMode(prev => {
      ...
      localStorage.setItem('odyssey_dashboard_view', next)
      return next
    })
  }
