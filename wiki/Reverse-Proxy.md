...
Putting ODYSSEY behind a TLS-terminating reverse proxy is strongly recommended for production.

## Why HTTPS Matters for ODYSSEY

- **PWA install** requires HTTPS — browsers block "Add to Home Screen" on plain HTTP.
- **Session cookies** — the `odyssey_session` cookie is marked `secure` in production, so it won't be sent over HTTP.
...
1. **WebSocket upgrades on `/ws`** — ODYSSEY uses WebSockets for real-time sync. Set `proxy_read_timeout 86400` (Nginx) or rely on Caddy's automatic upgrade handling.
...
server {
    listen 80;
    server_name odyssey.yourdomain.com;
...
- `X-Forwarded-Proto $scheme` — tells ODYSSEY whether the original request was HTTPS; required for `FORCE_HTTPS` redirect and cookie security to work correctly.
...
Four variables control how ODYSSEY behaves behind a proxy. They work as a group:
...
| `COOKIE_SECURE` | Controls the `secure` flag on `odyssey_session`. Auto-derived as `true` when `NODE_ENV=production` or `FORCE_HTTPS=true`. Set to `false` explicitly to allow cookies over plain HTTP (e.g. LAN testing without TLS). | auto |
| `ALLOWED_ORIGINS` | Comma-separated list of allowed CORS origins (e.g. `https://odyssey.example.com`). In production without this set, all cross-origin requests are blocked. In development without this set, all origins are allowed. | blocked in prod, open in dev |
...
If you access ODYSSEY directly on `http://<host>:3000` without a proxy, leave `FORCE_HTTPS` unset and do not set `TRUST_PROXY`.
