...
Go to the [ODYSSEY repository](https://github.com/mauriceboe/ODYSSEY) and click **Fork** to create your own copy.

Then clone your fork locally:

```bash
# Clone your fork, checking out the dev branch
git clone -b dev git@github.com:your-username/ODYSSEY.git
cd ODYSSEY
```
...
```bash
git remote add upstream git@github.com:mauriceboe/ODYSSEY.git
```
...
| Remote     | URL                                          | Purpose                        |
|------------|----------------------------------------------|--------------------------------|
| `origin`   | `git@github.com:your-username/ODYSSEY.git`      | Your fork — push changes here  |
| `upstream` | `git@github.com:mauriceboe/ODYSSEY.git`         | Main repo — pull updates from here |
...
The `@odyssey/shared` package is the single source of truth for code shared between the client and server. It currently holds **Zod schemas that define API contracts** (request/response shapes, common primitives, pagination). Both workspaces import from it so schema changes automatically propagate to both sides.
...
Then open a Pull Request from your fork to `mauriceboe/ODYSSEY` targeting the `dev` branch. If your PR only modifies files under `wiki/`, it is exempt from branch enforcement and may target any branch.
