...
How to update ODYSSEY to a newer version without losing data.
...
|---|---|---|
| `latest` | `mauriceboe/odyssey:latest` | Always the newest release across all major versions |
| Major version | `mauriceboe/odyssey:3` | Latest release pinned to that major version |
| Full version | `mauriceboe/odyssey:3.0.15` | Exact release; never changes |
...
If you started ODYSSEY with `docker run`, pull the new image and replace the container:

```bash
docker pull mauriceboe/odyssey
docker rm -f odyssey
docker run -d --name odyssey -p 3000:3000 \
  -v ./data:/app/data \
...
> ```bash
> docker inspect odyssey --format '{{json .Mounts}}'
> ```

## Database Migrations

ODYSSEY runs any pending database migrations automatically at startup. No manual migration steps are required after pulling a new image.

## Encryption Key Note

If you are upgrading from a version that predates the dedicated `ENCRYPTION_KEY` (i.e. you have no `ENCRYPTION_KEY` environment variable set), ODYSSEY automatically falls back to `./data/.jwt_secret` on startup and immediately promotes it to `./data/.encryption_key`. No manual steps are required — the transition is handled at first boot after the upgrade.
...
If you installed ODYSSEY via the [Proxmox VE Community Scripts](https://community-scripts.org/scripts/odyssey), run the following command inside the **LXC container** and select **Update** when prompted:
...
Open the **Stacks** list, click the ODYSSEY stack, then click **Redeploy**.
...
In the Unraid Docker tab, click the ODYSSEY container and select **Update**. Unraid will pull the latest image and restart with the same volumes.
