...
ODYSSEY exposes **tools** (read and write actions) and **resources** (read-only `odyssey://` URIs). Tools are registered per-session based on OAuth scopes and enabled addons.
...
Resources provide read-only access via `odyssey://` URIs. Read them to understand current state before making changes.

### Core resources

| URI | Scope required | Description |
|---|---|---|
| `odyssey://trips` | `trips:*` | All trips you own or are a member of |
| `odyssey://trips/{tripId}` | `trips:*` | Single trip with metadata and member count |
| `odyssey://trips/{tripId}/days` | `trips:*` | Days of a trip with their assigned places |
| `odyssey://trips/{tripId}/places` | `places:read` | All places in a trip. Supports `?assignment=all\|unassigned\|assigned` |
| `odyssey://trips/{tripId}/reservations` | `reservations:read` | Flights, hotels, restaurants, and other reservations |
| `odyssey://trips/{tripId}/days/{dayId}/notes` | `trips:*` | Notes for a specific day |
| `odyssey://trips/{tripId}/accommodations` | `trips:*` | Hotels and rentals with check-in/out details |
| `odyssey://trips/{tripId}/members` | `trips:*` | Owner and collaborators |
| `odyssey://categories` | (any) | Available place categories (id, name, icon, color) |
| `odyssey://notifications/in-app` | `notifications:read` | Your in-app notifications (most recent 50, unread first) |
