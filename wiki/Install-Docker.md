...
```bash
  -e TZ=Europe/Berlin \
  -e ALLOWED_ORIGINS=https://odyssey.example.com \
```
...
Replace `mauriceboe/odyssey:latest` in the run command with your chosen tag to pin to a major version or exact release.

## Volume Reference

| Volume | Container path | What lives there |
|---|---|---|
| `./data` | `/app/data` | `travel.db` (SQLite database), `logs/odyssey.log`, `.jwt_secret`, `.encryption_key` |
...
docker run -d \
  --name odyssey \
  -p 3000:3000 \
  -v odyssey_data:/app/data \
  -v odyssey_uploads:/app/uploads \
  -e ENCRYPTION_KEY=<your-32-byte-hex-key> \
  --restart unless-stopped \
  mauriceboe/odyssey:latest
```

Docker creates `odyssey_data` and `odyssey_uploads` automatically on first run. Named volumes are easier to manage with `docker volume` commands and work better in some NAS or container-management environments.
...
- [Reverse-Proxy](Reverse-Proxy) — HTTPS is required for PWA install and the `odyssey_session` cookie `secure` flag
