...
See https://github.com/mauriceboe/ODYSSEY/blob/main/docker-compose.yml
...
services:
  app:
    ...
    volumes:
      - odyssey_data:/app/data
      - odyssey_uploads:/app/uploads
...
```yaml
image: mauriceboe/odyssey:3        # track major version 3
image: mauriceboe/odyssey:3.0.15   # pin to exact release
```
...
This compose file is designed for deployments where a reverse proxy (nginx, Caddy, Traefik) terminates TLS in front of ODYSSEY. To enable HTTPS redirects and secure cookies, uncomment `FORCE_HTTPS=true` and `TRUST_PROXY=1`.
