...
Install ODYSSEY on Unraid via Community Applications or a direct template import.
...
2. Search for **ODYSSEY**.
3. Click **Install** on the ODYSSEY result.

If the app does not appear, you can install directly from the template URL. In **Docker → Add Container**, paste the template URL:

```
https://raw.githubusercontent.com/mauriceboe/ODYSSEY/main/unraid-template.xml
```
...
| Data | `/app/data` | `/mnt/user/appdata/odyssey/data` |
| Uploads | `/app/uploads` | `/mnt/user/appdata/odyssey/uploads` |
...
| `ALLOWED_ORIGINS` | *(empty)* | Comma-separated origins for CORS and email notification links, e.g. `https://odyssey.example.com` |
...
| `ADMIN_PASSWORD` | *(empty)* | Password for the first admin account (first-boot only). Must be set together with `ADMIN_EMAIL`. If either is omitted, ODYSSEY creates the account with email `admin@odyssey.local` and a random password printed to the container log. |
...
Copy the output into the `ENCRYPTION_KEY` field before starting the container for the first time. If you skip this, ODYSSEY auto-generates a key and saves it to `data/.encryption_key` — your data is still protected, but you should record that file in your backups.
...
On first boot, ODYSSEY automatically creates an admin account. The credentials are printed to the container log — check **Docker → odyssey → Log** in the Unraid UI. If you set both `ADMIN_EMAIL` and `ADMIN_PASSWORD`, those values are used; otherwise the email is `admin@odyssey.local` and a random password is generated.
