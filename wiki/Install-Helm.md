...
Deploy ODYSSEY on Kubernetes using the official Helm chart.

## Add the Chart Repository

```bash
helm repo add odyssey https://mauriceboe.github.io/ODYSSEY
helm repo update
```
...
```bash
helm install odyssey odyssey/odyssey
```

This deploys ODYSSEY with default values: a `ClusterIP` service on port 3000, 1 Gi PVCs for data and uploads, and no ingress.
...
```bash
helm install odyssey odyssey/odyssey --set generateEncryptionKey=true
```
...
```bash
kubectl create secret generic odyssey-secrets \
  --from-literal=ENCRYPTION_KEY=$(openssl rand -hex 32)

helm install odyssey odyssey/odyssey \
  --set existingSecret=odyssey-secrets
```
...
env:
  ...
  # ALLOWED_ORIGINS: "https://odyssey.example.com"
...
> **Important:** ODYSSEY uses WebSockets on `/ws`. Your ingress controller must support WebSocket upgrades. Set `proxy-read-timeout` to at least `86400` and `proxy-body-size` to at least `500m` for backup restores.
...
```bash
helm repo update
helm upgrade odyssey odyssey/odyssey
```

## Full Values Reference

See the [`charts/README.md`](https://github.com/mauriceboe/ODYSSEY/blob/main/charts/README.md) for all available values.
