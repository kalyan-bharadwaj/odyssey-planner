...
This page covers MCP tools and resources that require specific addons to be enabled on your ODYSSEY instance. For core tools (trips, places, day planning, accommodations, transport, reservations, budget, tags, maps, and notifications) see [MCP-Tools-and-Resources](MCP-Tools-and-Resources).
...
Resources provide read-only access via `odyssey://` URIs. The following resources require their addon to be enabled.

| URI | Addon | Scope required | Description |
|---|---|---|---|
| `odyssey://trips/{tripId}/budget` | Budget | `budget:read` | Budget and expense items |
| `odyssey://trips/{tripId}/budget/per-person` | Budget | `budget:read` | Per-person totals and split breakdown |
| `odyssey://trips/{tripId}/budget/settlement` | Budget | `budget:read` | Suggested transactions to settle who owes whom |
| `odyssey://trips/{tripId}/packing` | Packing | `packing:read` | Packing checklist |
| `odyssey://trips/{tripId}/packing/bags` | Packing | `packing:read` | Packing bags with their assigned members |
| `odyssey://trips/{tripId}/todos` | Packing | `todos:read` | To-do items ordered by position |
| `odyssey://trips/{tripId}/collab-notes` | Collab | `collab:read` | Shared collaborative notes |
| `odyssey://bucket-list` | Atlas | `atlas:read` | Your personal travel bucket list |
| `odyssey://visited-countries` | Atlas | `atlas:read` | Countries marked as visited in Atlas |
| `odyssey://atlas/stats` | Atlas | `atlas:read` | Visited country counts and continent breakdown |
| `odyssey://atlas/regions` | Atlas | `atlas:read` | Manually visited sub-country regions |
| `odyssey://trips/{tripId}/collab/polls` | Collab | `collab:read` | All polls for a trip with vote counts per option |
| `odyssey://trips/{tripId}/collab/messages` | Collab | `collab:read` | Most recent 100 chat messages for a trip |
| `odyssey://vacay/plan` | Vacay | `vacay:read` | Full snapshot of your active vacation plan (members, years, config) |
| `odyssey://vacay/entries/{year}` | Vacay | `vacay:read` | All vacation day entries for the active plan and a specific year |
| `odyssey://vacay/holidays/{year}` | Vacay | `vacay:read` | Public holidays for the plan's configured region and year |
| `odyssey://journeys` | Journey | `journey:read` | All journeys owned or contributed to by the current user |
| `odyssey://journeys/{journeyId}` | Journey | `journey:read` | Single journey with entries, contributors, and linked trips |
| `odyssey://journeys/{journeyId}/entries` | Journey | `journey:read` | All entries in a journey (date, text, mood, linked trip) |
| `odyssey://journeys/{journeyId}/contributors` | Journey | `journey:read` | Contributors (owner and collaborators) of a journey |
