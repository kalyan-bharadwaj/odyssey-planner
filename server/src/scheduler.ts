...
// Version check: daily at 9 AM — notify admins if a new ODYSSEY release is available
...
// The TTL must exceed any realistic offline window: the ODYSSEY client replays
...
// Odyssey photo cache cleanup: every 2 hours — evict disk files and DB rows past their 1h TTL
let odysseyPhotoCacheTask: ScheduledTask | null = null;

function startOdysseyPhotoCacheCleanup(): void {
  if (odysseyPhotoCacheTask) {
    odysseyPhotoCacheTask.stop();
    odysseyPhotoCacheTask = null;
  }

  // Run once immediately on startup to evict any entries left over from a previous run
  try {
    const { sweepExpired } = require('./services/memories/odysseyPhotoCache');
    sweepExpired();
  } catch {
    /* cache dir may not exist yet — harmless */
  }

  odysseyPhotoCacheTask = cron.schedule('0 */2 * * *', () => {
    try {
      const { sweepExpired } = require('./services/memories/odysseyPhotoCache');
      sweepExpired();
    } catch (err: unknown) {
      logError(`Odyssey photo cache cleanup: ${err instanceof Error ? err.message : err}`);
    }
  });
}
