...
import { EMAIL_I18N as I18N, EVENT_TEXTS, PASSWORD_RESET_I18N } from '@odyssey/shared/i18n/externalNotifications';
import type {
  EmailStrings,
  EventText,
  PasswordResetStrings,
  NotificationEventKey,
} from '@odyssey/shared/i18n/externalNotifications';
...
// ── Email i18n strings — imported from @odyssey/shared/i18n/externalNotifications ──

// EVENT_TEXTS imported from @odyssey/shared/i18n/externalNotifications
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 480px; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
        ...
        ${
          ...
          <a href="${ctaHref}" style="display: inline-block; padding: 12px 28px; background: #111827; color: #ffffff; font-size: 13px; font-weight: 600; text-decoration: none; border-radius: 10px; letter-spacing: 0.2px;">${s.openOdyssey}</a>
        </td></tr>`
            : ''
        }
        <!-- Footer -->
        <tr><td style="padding: 20px 32px; background: #f9fafb; border-top: 1px solid #f3f4f6; text-align: center;">
          ...
          <p style="margin: 0; font-size: 10px; color: #d1d5db;">${s.madeWith} <span style="color: #ef4444;">&hearts;</span> by Maurice &middot; <a href="https://github.com/mauriceboe/ODYSSEY" style="color: #9ca3af; text-decoration: none;">GitHub</a></p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
// PASSWORD_RESET_I18N imported from @odyssey/shared/i18n/externalNotifications
...
export async function sendPasswordResetEmail(
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtpCfg.from,
      to,
      subject: `ODYSSEY — ${strings.subject}`,
...
export function buildWebhookBody(
      ...
      embeds: [
        {
          ...
          url: payload.link,
          color: 0x3b82f6,
          footer: { text: payload.tripName ? `Trip: ${payload.tripName}` : 'ODYSSEY' },
  ...
  if (isSlack) {
    ...
    const link = payload.link ? `\n<${payload.link}|Open in ODYSSEY>` : '';
    return JSON.stringify({
      text: `*${payload.title}*\n${payload.body}${trip}${link}`,
    });
  }

  return JSON.stringify({ ...payload, timestamp: new Date().toISOString(), source: 'ODYSSEY' });
}
...
export async function testSmtp(to: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    await transporter.sendMail({
      from: config.from,
      to,
      subject: 'ODYSSEY — Test Notification',
      text: 'This is a test email from ODYSSEY. If you received this, your SMTP configuration is working correctly.',
    });
    return { success: true };
  } catch (err) {
...
export async function testWebhook(url: string): Promise<{ success: boolean; error?: string }> {
  try {
    const sent = await sendWebhook(url, {
      ...
      body: 'This is a test webhook from ODYSSEY. If you received this, your webhook configuration is working correctly.',
    });
...
export async function testNtfy(cfg: {
  ...
  try {
    const sent = await sendNtfy(url, cfg.token ?? null, {
      event: 'test',
      title: 'Test Notification',
      body: 'This is a test notification from ODYSSEY. If you received this, your ntfy configuration is working correctly.',
    });
