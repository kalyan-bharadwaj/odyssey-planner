...
export function exportICS(tripId: string | number): { ics: string; filename: string } {
  ...
  const uid = (id: number, type: string) => `odyssey-${type}-${id}@odyssey`;
  ...
  let ics =
    'BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//ODYSSEY//Travel Planner//EN\r\nCALSCALE:GREGORIAN\r\nMETHOD:PUBLISH\r\n';
  ics += `X-WR-CALNAME:${esc(trip.title || 'ODYSSEY Trip')}\r\n`;
  ...
  const safeFilename = (trip.title || 'odyssey-trip').replace(/["\r\n]/g, '').replace(/[^\w\s.-]/g, '_');
