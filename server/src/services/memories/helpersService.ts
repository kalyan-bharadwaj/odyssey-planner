...
export function canAccessUserPhoto(
    ...
    const journeyPhoto = db
      .prepare(
        `
            ...
            JOIN odyssey_photos tkp ON tkp.id = gp.photo_id
            WHERE tkp.asset_id = ?
              AND tkp.provider = ?
              AND tkp.owner_id = ?
            LIMIT 1
  ...
  // Regular trip photos — join through odyssey_photos
...
// ── Unified photo access check (odyssey_photos based) ──────────────────────

export function canAccessOdysseyPhoto(requestingUserId: number, odysseyPhotoId: number): boolean {
  const photo = db.prepare('SELECT * FROM odyssey_photos WHERE id = ?').get(odysseyPhotoId) as
    | { id: number; provider: string; owner_id: number | null }
    | undefined;
