...
import type { OdysseyPhoto } from '../../types';
...
import * as photoCache from './odysseyPhotoCache';
...
export function getOrCreateOdysseyPhoto(provider: string, assetId: string, ownerId: number, passphrase?: string): number {
  const existing = db
  ...
  if (existing) {
    if (passphrase) {
      db.prepare('UPDATE odyssey_photos SET passphrase = ? WHERE id = ?').run(encrypt_api_key(passphrase), existing.id);
    }
    return existing.id;
  }

  const res = db
    .prepare('INSERT INTO odyssey_photos (provider, asset_id, owner_id, passphrase) VALUES (?, ?, ?, ?)')
    .run(provider, assetId, ownerId, passphrase ? encrypt_api_key(passphrase) : null);
  return Number(res.lastInsertRowid);
}

export function getOrCreateLocalOdysseyPhoto(
  filePath: string,
  thumbnailPath?: string | null,
  width?: number | null,
  height?: number | null,
): number {
  const existing = db.prepare("SELECT id FROM odyssey_photos WHERE provider = 'local' AND file_path = ?").get(filePath) as
    | { id: number }
    | undefined;
  if (existing) return existing.id;

  const res = db
    .prepare('INSERT INTO odyssey_photos (provider, file_path, thumbnail_path, width, height) VALUES (?, ?, ?, ?, ?)')
    .run('local', filePath, thumbnailPath || null, width || null, height || null);
  return Number(res.lastInsertRowid);
}

export function resolveOdysseyPhoto(photoId: number): OdysseyPhoto | null {
  return (db.prepare('SELECT * FROM odyssey_photos WHERE id = ?').get(photoId) as OdysseyPhoto | undefined) || null;
}
...
export async function streamPhoto(
  ...
  const photo = resolveOdysseyPhoto(photoId);
  if (!photo) {
    ...
    if (kind === 'thumbnail') {
      ...
      if (!thumbRel) {
        ...
        if (result) {
          ...
          db.prepare(
            'UPDATE odyssey_photos SET thumbnail_path = ?, width = COALESCE(width, ?), height = COALESCE(height, ?) WHERE id = ?',
...
// ── Update provider on existing odyssey_photo (for Immich upload sync) ─────

export function setOdysseyPhotoProvider(odysseyPhotoId: number, provider: string, assetId: string, ownerId: number): void {
  db.prepare('UPDATE odyssey_photos SET provider = ?, asset_id = ?, owner_id = ? WHERE id = ?').run(
    provider,
    assetId,
    ownerId,
    odysseyPhotoId,
...
export function deleteOdysseyPhotoIfOrphan(photoId: number): void {
  const stillUsed = db
    .prepare(
      `
  ...
  `,
    ...
    .get(photoId, photoId);
  if (stillUsed) return;
  db.prepare("DELETE FROM odyssey_photos WHERE id = ? AND provider != 'local'").run(photoId);
}
