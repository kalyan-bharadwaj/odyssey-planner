...
import { getOrCreateOdysseyPhoto, deleteOdysseyPhotoIfOrphan } from './photoResolverService';
...
function _addTripPhoto(
  ...
  try {
    const photoId = getOrCreateOdysseyPhoto(provider, assetId, userId, passphrase);
    const result = db
      .prepare(
...
export function removeTripPhoto(tripId: string, userId: number, photoId: number, sid?: string): ServiceResult<true> {
  ...
  try {
    ...
    deleteOdysseyPhotoIfOrphan(photoId);
    broadcast(tripId, 'memories:updated', { userId }, sid);
...
export function removeAlbumLink(tripId: string, linkId: string, userId: number): ServiceResult<true> {
  ...
  try {
    ...
    for (const { photo_id } of linkedPhotos) {
      deleteOdysseyPhotoIfOrphan(photo_id);
    }
