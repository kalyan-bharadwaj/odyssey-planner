...
function _getSynologyCredentials(userId: number): ServiceResult<SynologyCredentials> {
  ...
  if (!user?.data.synology_url || !user.data.synology_username || !user.data.synology_password)
    return fail('Synology not configured', 400);
...
const SYNOLOGY_DEVICE_NAME = 'odyssey';
...
async function _requestSynologyApi<T>(userId: number, params: ApiCallParams): Promise<ServiceResult<T>> {
  ...
  if ('error' in result && [106, 107, 119].includes(result.error.status)) {
    ...
    return _fetchSynologyJson<T>(
      creds.data.synology_url,
      _buildSynologyFormBody({ ...params, _sid: retrySession.data }),
      skipSsl,
    );
  }
  return result;
...
async function _getSynologySession(userId: number): Promise<ServiceResult<string>> {
  ...
  const storedDid =
    cached.success && cached.data?.synology_did ? decrypt_api_key(cached.data.synology_did) || undefined : undefined;

  const resp = await _loginToSynology(
    creds.data.synology_url,
    creds.data.synology_username,
    creds.data.synology_password,
    {
      deviceId: storedDid,
...
export async function updateSynologySettings(
  userId: number,
  synologyUrl: string,
  synologyUsername: string,
  synologyPassword?: string,
  synologySkipSsl = false,
...
export async function listSynologyAlbums(userId: number): Promise<ServiceResult<AlbumsList>> {
  const [personal, shared, sharedWithMe] = await Promise.allSettled([
    ...
    _fetchAllSynologyAlbums(userId, {
  ...
  const addAlbums = (
    result: PromiseSettledResult<ServiceResult<any[]>>,
    extractPassphrase: (a: any) => string | undefined,
  ) => {
...
export async function getSynologyAlbumPhotos(
  userId: number,
  albumId: string,
  passphrase?: string,
): Promise<ServiceResult<AssetsList>> {
...
export async function syncSynologyAlbumLink(
  userId: number,
  tripId: string,
  linkId: string,
  sid: string,
...
export async function searchSynologyPhotos(
  userId: number,
  from?: string,
  to?: string,
  offset = 0,
...
export async function getSynologyAssetInfo(
  userId: number,
  photoId: string,
  targetUserId: number,
  passphrase?: string,
