...
const ODYSSEY_PHOTO_DIR = path.join(__dirname, '../../../uploads/photos/odyssey');
...
function ensureDir(): void {
  if (!fs.existsSync(ODYSSEY_PHOTO_DIR)) {
    fs.mkdirSync(ODYSSEY_PHOTO_DIR, { recursive: true });
  }
}

function cachedFilePath(key: string): string {
  return path.join(ODYSSEY_PHOTO_DIR, `${key}.bin`);
}

export function getFresh(key: string): { filePath: string; contentType: string } | null {
  const row = db.prepare('SELECT content_type, fetched_at FROM odyssey_photo_cache_meta WHERE cache_key = ?').get(key) as
  ...
  if (Date.now() - row.fetched_at >= CACHE_TTL) {
    db.prepare('DELETE FROM odyssey_photo_cache_meta WHERE cache_key = ?').run(key);
    return null;
  }
...
export async function put(key: string, bytes: Buffer, contentType: string): Promise<void> {
  ...
  db.prepare('INSERT OR REPLACE INTO odyssey_photo_cache_meta (cache_key, content_type, fetched_at) VALUES (?, ?, ?)').run(
    key,
    contentType,
    Date.now(),
  );
...
export function sweepExpired(): void {
  ...
  const stale = db.prepare('SELECT cache_key FROM odyssey_photo_cache_meta WHERE fetched_at < ?').all(cutoff) as {
    cache_key: string;
  }[];

  for (const row of stale) {
    db.prepare('DELETE FROM odyssey_photo_cache_meta WHERE cache_key = ?').run(row.cache_key);
