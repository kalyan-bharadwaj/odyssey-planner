...
function generateAccessToken(): string {
  return 'odysseyoa_' + randomBytes(32).toString('hex');
}

function generateRefreshToken(): string {
  return 'odysseyrf_' + randomBytes(32).toString('hex');
}
...
export function createOAuthClient(
  ...
  const rawSecret = isPublic ? null : 'odysseycs_' + randomBytes(24).toString('hex');
...
export function rotateOAuthClientSecret(
  ...
  const rawSecret = 'odysseycs_' + randomBytes(24).toString('hex');
...
export function validateAuthorizeRequest(params: AuthorizeParams, userId: number | null): ValidateAuthorizeResult {
  ...
  // RFC 8707 resource indicator: if provided, must identify the ODYSSEY
  ...
  if (resource !== mcpResource) {
    return {
      ...
      error_description: 'Requested resource must be the ODYSSEY MCP endpoint',
    };
  }
