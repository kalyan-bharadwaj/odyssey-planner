...
import { CONTINENT_MAP } from '@odyssey/shared';
...
export const COUNTRY_BOXES: Record<string, [number, number, number, number]> = {
  AF: [60.5, 29.4, 75, 38.5],
  AL: [19, 39.6, 21.1, 42.7],
  DZ: [-8.7, 19, 12, 37.1],
  AD: [1.4, 42.4, 1.8, 42.7],
  AO: [11.7, -18.1, 24.1, -4.4],
  ...
  BF: [-5.5, 9.4, 2.4, 15.1],
  BH: [50.4, 25.8, 50.7, 26.2],
  BI: [29.0, -4.5, 30.8, -2.3],
  BJ: [0.8, 6.2, 3.8, 12.4],
  BN: [114.1, 4.0, 115.4, 5.1],
  BO: [-69.7, -22.9, -57.5, -9.7],
  ...
  VA: [12.44, 41.9, 12.46, 41.91],
  MC: [7.4, 43.72, 7.44, 43.75],
...
export async function reverseGeocodeCountry(lat: number, lng: number): Promise<string | null> {
  ...
  try {
    const res = await fetch(
      ...
      {
        headers: { 'User-Agent': 'ODYSSEY Travel Planner (https://github.com/mauriceboe/ODYSSEY)' },
