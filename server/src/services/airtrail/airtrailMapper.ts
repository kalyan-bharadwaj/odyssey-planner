...
import type { AirtrailFlight } from '@odyssey/shared';
 ...
 * `date`; the airport-local wall time is what ODYSSEY shows and files days by.
 */
...
export function mapFlightToReservation(raw: AirtrailFlightRaw): MappedReservation {
  // Read the SCHEDULED times only — ODYSSEY plans against the scheduled (booked) time,
 ...
 * remote changes (AirTrail exposes no updated_at/etag) and to suppress ODYSSEY's own
