 ...
 * updated_at) and, when a flight is gone from AirTrail, keep the ODYSSEY row but
 ...
 * GET object) and overwrite ONLY the fields ODYSSEY manages. Everything else —
...
export function buildSavePayload(reservation: any, existing: AirtrailFlightRaw): AirtrailSavePayload | null {
  ...
  // Push the seat the user set in ODYSSEY onto their own AirTrail seat (the one with
  ...
  // overwrite only what ODYSSEY manages. `from`/`to`/`airline`/`aircraft` come back
  ...
  return {
    ...
    // Import reads the SCHEDULED time, so a ODYSSEY edit must write back there too —
    ...
    // These are AirTrail-owned details ODYSSEY doesn't surface in its edit UI — a ODYSSEY
    // edit can leave them out of `metadata`. Preserve AirTrail's current value when
    // ODYSSEY has none rather than nulling it out (#1240). entityCode mirrors the
