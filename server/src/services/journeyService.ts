...
import {
  getOrCreateOdysseyPhoto,
  getOrCreateLocalOdysseyPhoto,
  setOdysseyPhotoProvider,
  deleteOdysseyPhotoIfOrphan,
...
// Per-entry photo view: join journey_entry_photos → journey_photos (gallery) → odyssey_photos.
...
const JP_JOIN = `journey_entry_photos jep
  ...
  JOIN odyssey_photos    tp ON tp.id  = gp.photo_id`;

// Per-journey gallery view: journey_photos → odyssey_photos (no entry context).
const GALLERY_SELECT = `
  gp.id, gp.journey_id, gp.photo_id, gp.caption, gp.shared, gp.sort_order, gp.created_at,
  tp.provider, tp.asset_id, tp.owner_id, tp.file_path, tp.thumbnail_path, tp.width, tp.height
`;
const GALLERY_JOIN = 'journey_photos gp JOIN odyssey_photos tp ON tp.id = gp.photo_id';
...
// Ensure a odyssey_photo_id is in the journey gallery; return its gallery row id.
function ensureInGallery(journeyId: number, odysseyPhotoId: number, caption?: string, shared?: number): number {
  const now = ts();
  const maxOrderRow = db
    .prepare('SELECT MAX(sort_order) as m FROM journey_photos WHERE journey_id = ?')
    .get(journeyId) as { m: number | null };
  db.prepare(
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `,
  ).run(journeyId, odysseyPhotoId, caption || null, shared ?? 0, (maxOrderRow?.m ?? -1) + 1, now);
  const row = db
    .prepare('SELECT id FROM journey_photos WHERE journey_id = ? AND photo_id = ?')
    .get(journeyId, odysseyPhotoId) as { id: number };
  return row.id;
}
...
export function addPhoto(
  ...
  const odysseyPhotoId = getOrCreateLocalOdysseyPhoto(filePath, thumbnailPath);
  const galleryId = db.transaction(() => ensureInGallery(entry.journey_id, odysseyPhotoId, caption))();
...
export function addProviderPhoto(
  ...
  const odysseyPhotoId = getOrCreateOdysseyPhoto(provider, assetId, userId, passphrase);
...
export function uploadGalleryPhotos(
  ...
  for (const f of filePaths) {
    const odysseyPhotoId = getOrCreateLocalOdysseyPhoto(f.path, f.thumbnail);
    db.prepare(
      `
      INSERT OR IGNORE INTO journey_photos (journey_id, photo_id, shared, sort_order, created_at)
      VALUES (?, ?, 0, ?, ?)
    `,
    ).run(journeyId, odysseyPhotoId, nextOrder++, now);
    const row = db
      .prepare(`SELECT ${GALLERY_SELECT} FROM ${GALLERY_JOIN} WHERE gp.journey_id = ? AND gp.photo_id = ?`)
      .get(journeyId, odysseyPhotoId);
...
export function addProviderPhotoToGallery(
  ...
  if (!canEdit(journeyId, userId)) return null;
  const odysseyPhotoId = getOrCreateOdysseyPhoto(provider, assetId, userId, passphrase);
  const galleryId = db.transaction(() => ensureInGallery(journeyId, odysseyPhotoId, caption))();
...
export function deleteGalleryPhoto(
  ...
  const odysseyRow = db.prepare('SELECT file_path, provider FROM odyssey_photos WHERE id = ?').get(row.photo_id) as
  ...
  deleteOdysseyPhotoIfOrphan(row.photo_id);

  return { photo_id: row.photo_id, file_path: odysseyRow?.file_path ?? null };
}

export function setPhotoProvider(photoId: number, provider: string, assetId: string, ownerId: number) {
  // photoId = journey_photos.id (gallery row); look up the odyssey_photo_id
  const jp = db.prepare('SELECT photo_id FROM journey_photos WHERE id = ?').get(photoId) as
    | { photo_id: number }
    | undefined;
  if (!jp) return;
  setOdysseyPhotoProvider(jp.photo_id, provider, assetId, ownerId);
...
export function deletePhoto(
  ...
  return { id: row.id, photo_id: row.photo_id, file_path: odysseyRow?.file_path ?? null, journey_id: row.journey_id };
}
