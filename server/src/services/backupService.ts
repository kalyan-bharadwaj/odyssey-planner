  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      if (fs.existsSync(uploadsDir)) {
        // Exclude the place-photo and odyssey-memory caches: both are re-derivable
        ...
        archive.glob(
          '**/*',
          { cwd: uploadsDir, ignore: ['photos/google/**', 'photos/odyssey/**'], nodir: true, dot: true },
          { prefix: 'uploads' },
        );
      }
    ...
    try {
      ...
      for (const table of requiredTables) {
        if (!tableNames.has(table)) {
          ...
          return {
            success: false,
            error: `Uploaded database is missing required table: ${table}. This does not appear to be a ODYSSEY backup.`,
            status: 400,
          };
        }
      }
