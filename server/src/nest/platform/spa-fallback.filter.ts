 ...
 * client) keeps the standard ODYSSEY `{ error }` 404 envelope. The `@Catch(NotFoundException)`
 * is more specific than the global OdysseyExceptionFilter, so Nest routes 404s here
 * while every other error still flows through OdysseyExceptionFilter.
 */
...
export class SpaFallbackFilter implements ExceptionFilter {
    ...
    // Non-production, or a non-GET miss: keep the standard ODYSSEY 404 envelope
    // (identical to what OdysseyExceptionFilter produces for a NotFoundException).
