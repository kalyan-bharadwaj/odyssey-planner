...
import { odysseyOAuthProvider, odysseyClientsStore } from '../../mcp/oauthProvider';
...
export function applyPlatformTransport(app: express.Application): void {
  ...
  // the ODYSSEY home page instead of the consent form.
  ...
  app.use('/oauth/authorize', mcpAddonGate, authorizationHandler({ provider: odysseyOAuthProvider }));

  // SDK DCR handler: accepts registrations without scope (fixes issue #959 bug 2)
  app.use('/oauth/register', mcpAddonGate, clientRegistrationHandler({ clientsStore: odysseyClientsStore }));
