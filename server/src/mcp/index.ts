...
You are connected to ODYSSEY, a travel planning application. Below is a compact reference of the data model, key workflows, and behavioral rules you must follow.
...
The following features are optional and may not be available on every ODYSSEY instance. Check tool availability before assuming they exist:
...
const STATIC_TOKEN_DEPRECATION_NOTICE =
  ...
  '"Your ODYSSEY integration is using a static API token that is deprecated and will stop working in a future version. ' +
  'Please migrate to OAuth 2.1: go to Settings → Integrations → MCP → OAuth Clients in ODYSSEY and register an OAuth 2.1 application." ' +
...
function setAuthChallenge(res: Response, error = 'invalid_token'): void {
  ...
  res.set(
    'WWW-Authenticate',
    `Bearer realm="ODYSSEY MCP", resource_metadata="${base}/.well-known/oauth-protected-resource/mcp", error="${error}"`,
  );
}
...
function verifyToken(authHeader: string | undefined): VerifyTokenResult | null {
  ...
  // OAuth 2.1 access token (odysseyoa_...)
  if (token.startsWith('odysseyoa_')) {
  ...
  // Long-lived static MCP token (odyssey_...) — full access + deprecation notice
  if (token.startsWith('odyssey_')) {
    const user = verifyMcpToken(token);
    if (!user) return null;
    return { user, scopes: null, clientId: null, isStaticToken: true };
  }

  // Short-lived JWT (ODYSSEY web session used directly) — full access, no notice
