 ...
 *
 * Re-encrypts all at-rest secrets in the ODYSSEY database from one ENCRYPTION_KEY
...
async function main() {
  console.log('=== ODYSSEY Encryption Key Migration ===\n');
  ...
  db.transaction(() => {
    ...
    const photos = db.prepare('SELECT id, passphrase FROM odyssey_photos WHERE passphrase IS NOT NULL').all() as { id: number; passphrase: string }[];
    for (const row of photos) {
      const newVal = migrateApiKeyValue(row.passphrase, `odyssey_photos[${row.id}].passphrase`);
      if (newVal !== null) {
        db.prepare('UPDATE odyssey_photos SET passphrase = ? WHERE id = ?').run(newVal, row.id);
      }
    }
  })();
