...
export function createMcpToken(
  ...
  const rawToken = overrides.rawToken ?? `odyssey_test_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
export function addTripPhoto(
  ...
  // Insert into odyssey_photos first (central registry)
  db.prepare('INSERT OR IGNORE INTO odyssey_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)').run(
    provider,
    assetId,
    userId,
  );
  const odysseyPhoto = db
  ...
  const result = db
    ...
    .run(tripId, userId, odysseyPhoto.id, opts.shared ? 1 : 0, opts.albumLinkId ?? null);
  return db
    .prepare(
      `
