...
import { getOrCreateOdysseyPhoto, deleteOdysseyPhotoIfOrphan } from '../../src/services/memories/photoResolverService';
...
vi.mock('../../src/config', () => ({
  JWT_SECRET: 'test-jwt-secret-for-odyssey-testing-only',
...
describe('Synology asset access', () => {
  ...
  it('SYNO-032b — GET /api/photos/:id/thumbnail uses an allowed Synology thumbnail size', async () => {
    ...
    const insert = testDb
      .prepare('INSERT INTO odyssey_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)')
      .run('synologyphotos', '101_cachekey', user.id);
    const odysseyPhotoId = Number(insert.lastInsertRowid);

    vi.mocked(safeFetch).mockClear();

    const res = await request(app).get(`/api/photos/${odysseyPhotoId}/thumbnail`).set('Cookie', authCookie(user.id));
  ...
  it('SYNO-035 — GET /assets/info where trip does not exist returns 403', async () => {
    ...
    testDb
      .prepare('INSERT OR IGNORE INTO odyssey_photos (provider, asset_id, owner_id) VALUES (?, ?, ?)')
      .run('synologyphotos', '101_cachekey', owner.id);
    const tkpSyno35 = testDb
      .prepare('SELECT id FROM odyssey_photos WHERE provider = ? AND asset_id = ? AND owner_id = ?')
...
describe('Synology syncSynologyAlbumLink', () => {
  it('SYNO-050 — POST sync happy path: trip owner with album link saves photos to DB', async () => {
    ...
    const photos = testDb
      ...
      JOIN odyssey_photos tkp ON tkp.id = tp.photo_id
  ...
  it('SYNO-054 — POST sync with passphrase link: uses passphrase in item-list call and persists encrypted passphrase on odyssey_photos', async () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    setSynologyCredentials(testDb, user.id, 'https://synology.example.com', 'admin', 'pass');
    testDb.prepare("UPDATE photo_providers SET enabled = 1 WHERE id = 'synologyphotos'").run();
    ...
    // The odyssey_photos row for the synced photo must have a non-null passphrase.
    const photo = testDb
      .prepare(
        `
...
describe('odyssey_photos passphrase healing (SYNO-090)', () => {
  it('SYNO-090 — getOrCreateOdysseyPhoto overwrites an existing bad passphrase when a new one is supplied', () => {
    const { user } = createUser(testDb);

    const wrongPass = 'wrong-passphrase';
    const correctPass = 'correct-passphrase';

    const id1 = getOrCreateOdysseyPhoto('synologyphotos', 'asset-heal-test', user.id, wrongPass);
    const row1 = testDb.prepare('SELECT passphrase FROM odyssey_photos WHERE id = ?').get(id1) as { passphrase: string };
    expect(decrypt_api_key(row1.passphrase)).toBe(wrongPass);

    const id2 = getOrCreateOdysseyPhoto('synologyphotos', 'asset-heal-test', user.id, correctPass);
    expect(id2).toBe(id1);
    const row2 = testDb.prepare('SELECT passphrase FROM odyssey_photos WHERE id = ?').get(id2) as { passphrase: string };
    expect(decrypt_api_key(row2.passphrase)).toBe(correctPass);
  });
});

describe('odyssey_photos orphan cleanup (SYNO-091)', () => {
  it('SYNO-091 — deleteOdysseyPhotoIfOrphan removes the odyssey_photos row when no trip_photos or journey_photos reference it', () => {
    const { user } = createUser(testDb);
    const trip = createTrip(testDb, user.id);
    testDb.prepare("UPDATE photo_providers SET enabled = 1 WHERE id = 'synologyphotos'").run();

    const odysseyPhotoId = getOrCreateOdysseyPhoto('synologyphotos', 'asset-orphan-test', user.id, 'pass-A');

    testDb
      .prepare('INSERT OR IGNORE INTO trip_photos (trip_id, user_id, photo_id, shared) VALUES (?, ?, ?, 1)')
      .run(trip.id, user.id, odysseyPhotoId);

    // Still referenced — must not be deleted.
    deleteOdysseyPhotoIfOrphan(odysseyPhotoId);
    expect(testDb.prepare('SELECT id FROM odyssey_photos WHERE id = ?').get(odysseyPhotoId)).toBeDefined();

    // Remove the reference, then orphan-cleanup should delete the odyssey_photos row.
    testDb.prepare('DELETE FROM trip_photos WHERE photo_id = ?').run(odysseyPhotoId);
    deleteOdysseyPhotoIfOrphan(odysseyPhotoId);
    expect(testDb.prepare('SELECT id FROM odyssey_photos WHERE id = ?').get(odysseyPhotoId)).toBeUndefined();
  });
  ...
  it('SYNO-092 — re-adding a previously removed Synology photo stores the new passphrase correctly', () => {
    ...
    const id1 = getOrCreateOdysseyPhoto('synologyphotos', 'asset-readd-test', user.id, firstPass);
    testDb
      .prepare('INSERT OR IGNORE INTO trip_photos (trip_id, user_id, photo_id, shared) VALUES (?, ?, ?, 1)')
      .run(trip.id, user.id, id1);
    testDb.prepare('DELETE FROM trip_photos WHERE photo_id = ?').run(id1);
    deleteOdysseyPhotoIfOrphan(id1);

    // odyssey_photos row should be gone.
    expect(testDb.prepare('SELECT id FROM odyssey_photos WHERE id = ?').get(id1)).toBeUndefined();

    // Re-add with the correct passphrase.
    const id2 = getOrCreateOdysseyPhoto('synologyphotos', 'asset-readd-test', user.id, secondPass);
    const row = testDb.prepare('SELECT passphrase FROM odyssey_photos WHERE id = ?').get(id2) as { passphrase: string };
