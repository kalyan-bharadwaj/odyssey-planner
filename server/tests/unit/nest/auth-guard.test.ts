...
describe('CookieAuthGuard', () => {
  ...
  it('401s when the odyssey_session cookie is missing', () => {
  ...
  it('401s when the cookie token fails verification', () => {
    vi.mocked(verifyJwtAndLoadUser).mockReturnValue(null);
    expect(thrown(() => guard.canActivate(context({ cookies: { odyssey_session: 'tok' } })))).toEqual({
      status: 401,
  ...
  it('attaches the user and allows a valid cookie session through', () => {
    const req: Record<string, unknown> = { cookies: { odyssey_session: 'tok' } };
    vi.mocked(verifyJwtAndLoadUser).mockReturnValue(user);
    expect(guard.canActivate(context(req))).toBe(true);
    expect(req.user).toBe(user);
  });
...
describe('PasskeyController', () => {
  ...
  it('register/options maps a service error, else returns the options', async () => {
    ...
    expect(await thrownAsync(() => new PasskeyController(rl()).registerOptions(user, { password: 'x' }, req))).toEqual({
      status: 401,
  ...
  it('register/verify maps a service error, else audits and returns the credential', async () => {
    ...
    expect(await thrownAsync(() => new PasskeyController(rl()).registerVerify(user, {}, req))).toEqual({
      status: 400,
      body: { error: 'Verification failed' },
    });
    vi.mocked(passkey.passkeyRegisterVerify).mockResolvedValue({ credential: { id: 'cr' } } as never);
    expect(await new PasskeyController(rl()).registerVerify(user, {}, req)).toEqual({
      success: true,
      credential: { id: 'cr' },
    });
  ...
  it('login/options maps a service error, else returns the options', async () => {
    ...
    expect(await thrownAsync(() => new PasskeyController(rl()).loginOptions(req))).toEqual({
      status: 503,
  ...
  it('login/verify audits a failure then maps the error, padding latency', async () => {
    vi.mocked(passkey.passkeyLoginVerify).mockResolvedValue({
      error: 'No match',
      status: 401,
      auditAction: 'user.login_fail',
      auditUserId: null,
    } as never);
    expect(await thrownAsync(() => new PasskeyController(rl()).loginVerify({}, req, res))).toEqual({
      status: 401,
      body: { error: 'No match' },
    });
  ...
  it('credentials: list, rename (error + success), delete (error + success)', () => {
    ...
    expect(thrown(() => new PasskeyController(rl()).rename(user, 'cid', { name: 'x' }))).toEqual({
      status: 404,
    ...
    expect(thrown(() => new PasskeyController(rl()).remove(user, 'cid', { password: 'x' }, req))).toEqual({
      status: 401,
      body: { error: 'Incorrect password' },
    });
  ...
  it('throttles registration and login ceremonies once the bucket is exhausted', async () => {
    ...
    expect(await thrownAsync(() => new PasskeyController(s).registerOptions(user, {}, req))).toEqual({
      status: 429,
    ...
    expect(await thrownAsync(() => new PasskeyController(s2).loginOptions(req))).toEqual({
      status: 429,
      body: { error: 'Too many attempts. Please try again later.' },
    });
  });
