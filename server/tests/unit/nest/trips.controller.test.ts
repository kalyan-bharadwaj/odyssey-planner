...
describe('TripsController (parity with the legacy /api/trips route)', () => {
  ...
  describe('POST / (create)', () => {
    it('403 without trip_create, 400 without title', () => {
      expect(
        thrown(() =>
          new TripsController(svc({ can: vi.fn().mockReturnValue(false) })).create(user, { title: 'T' }, req),
        ),
      ).toEqual({ status: 403, body: { error: 'No permission to create trips' } });
      expect(thrown(() => new TripsController(svc()).create(user, {}, req))).toEqual({
        status: 400,
    ...
    it('400 when end_date precedes start_date', () => {
      expect(
        thrown(() =>
          new TripsController(svc()).create(
            user,
            { title: 'T', start_date: '2026-07-10', end_date: '2026-07-01' },
            req,
          ),
        ),
      ).toEqual({
  ...
  it('GET /:id 404 when missing', () => {
    expect(
      thrown(() =>
        new TripsController(svc({ get: vi.fn().mockReturnValue(undefined) } as Partial<TripsService>)).get(user, '9'),
      ),
  ...
  describe('PUT /:id', () => {
    it('404 when no access; 403 on archive without trip_archive', () => {
      expect(
        thrown(() =>
          new TripsController(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).update(user, '9', {}, req),
        ),
      ).toEqual({ status: 404, body: { error: 'Trip not found' } });
      const s = svc({ can: vi.fn().mockImplementation((a: string) => a !== 'trip_archive') });
      expect(thrown(() => new TripsController(s).update(user, '9', { is_archived: 1 }, req))).toEqual({
        status: 403,
    ...
    it('403 on cover_image without trip_cover_upload', () => {
      ...
      expect(thrown(() => new TripsController(s).update(user, '9', { cover_image: '/x.jpg' }, req))).toEqual({
        status: 403,
    ...
    it('403 on an edit field without trip_edit', () => {
      ...
      expect(thrown(() => new TripsController(s).update(user, '9', { title: 'b' }, req))).toEqual({
        status: 403,
    ...
    it('maps a NotFoundError to 404 and a ValidationError to 400', () => {
      ...
      expect(thrown(() => new TripsController(nf).update(user, '9', { title: 'b' }, req))).toEqual({
        status: 404,
        body: { error: 'gone' },
      });
      const ve = svc({
      ...
      expect(thrown(() => new TripsController(ve).update(user, '9', { title: 'b' }, req))).toEqual({
        status: 400,
        body: { error: 'bad' },
      });
    });
  ...
  describe('POST /:id/copy', () => {
    it('403 without trip_create, 404 without access', () => {
      expect(
        thrown(() => new TripsController(svc({ can: vi.fn().mockReturnValue(false) })).copy(user, '9', undefined, req)),
      ).toEqual({ status: 403, body: { error: 'No permission to create trips' } });
      expect(
        thrown(() =>
          new TripsController(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).copy(
            user,
            '9',
            undefined,
            req,
  ...
  describe('DELETE /:id', () => {
    it('404 when no owner, 403 without trip_delete', () => {
      expect(
        thrown(() =>
          new TripsController(svc({ getOwner: vi.fn().mockReturnValue(undefined) } as Partial<TripsService>)).remove(
            user,
            '9',
            req,
          ),
      ...
      expect(thrown(() => new TripsController(s).remove(user, '9', req))).toEqual({
        status: 403,
  ...
  describe('members', () => {
    it('GET 404 without access, else owner+members+current_user_id', () => {
      expect(
        thrown(() =>
          new TripsController(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).members(user, '9'),
        ),
      ).toEqual({ status: 404, body: { error: 'Trip not found' } });
      const s = svc({
        listMembers: vi.fn().mockReturnValue({ owner: { id: 1 }, members: [] }),
      } as Partial<TripsService>);
    ...
    it('POST 403 without member_manage, else adds + notifies', () => {
      expect(
        thrown(() => new TripsController(svc({ can: vi.fn().mockReturnValue(false) })).addMember(user, '9', 'bob@x.y')),
      ).toEqual({ status: 403, body: { error: 'No permission to manage members' } });
      const addMember = vi
        .fn()
        .mockReturnValue({ member: { id: 2, email: 'bob@x.y' }, targetUserId: 2, tripTitle: 'T' });
    ...
    it('POST 404 without trip access', () => {
      ...
      expect(thrown(() => new TripsController(s).addMember(user, '9', 'bob@x.y'))).toEqual({
        status: 404,
    ...
    it('POST maps NotFoundError to 404, ValidationError to 400, re-throws others', () => {
      ...
      expect(thrown(() => new TripsController(nf).addMember(user, '9', 'bob@x.y'))).toEqual({
        status: 404,
        body: { error: 'no user' },
      });
      const ve = svc({
      ...
      expect(thrown(() => new TripsController(ve).addMember(user, '9', 'bob@x.y'))).toEqual({
        status: 400,
    ...
    it('DELETE 404 without trip access', () => {
      const s = svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) });
      expect(thrown(() => new TripsController(s).removeMember(user, '9', '2'))).toEqual({
        status: 404,
        body: { error: 'Trip not found' },
      });
    });
    ...
    it('DELETE self needs no permission; removing others needs member_manage', () => {
      ...
      expect(thrown(() => new TripsController(s).removeMember(user, '9', '2'))).toEqual({
        status: 403,
        body: { error: 'No permission to remove members' },
      });
    });
  });
  ...
  it('GET /:id/bundle 404 then aggregates', () => {
    expect(
      thrown(() =>
        new TripsController(svc({ get: vi.fn().mockReturnValue(undefined) } as Partial<TripsService>)).bundle(
          user,
          '9',
        ),
      ),
  ...
  describe('POST /:id/cover', () => {
    ...
    it('404 without access, 403 without permission, 404 raw trip, 400 no file, else returns url', () => {
      expect(
        thrown(() =>
          new TripsController(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).cover(user, '9', file),
        ),
      ).toEqual({ status: 404, body: { error: 'Trip not found' } });
      expect(
        thrown(() => new TripsController(svc({ can: vi.fn().mockReturnValue(false) })).cover(user, '9', file)),
      ).toEqual({ status: 403, body: { error: 'No permission to change the cover image' } });
      expect(
        thrown(() =>
          new TripsController(svc({ getRaw: vi.fn().mockReturnValue(undefined) } as Partial<TripsService>)).cover(
            user,
            '9',
            file,
          ),
      ...
      expect(
        thrown(() =>
          new TripsController(
            svc({ getRaw: vi.fn().mockReturnValue({ cover_image: null }) } as Partial<TripsService>),
          ).cover(user, '9', undefined),
        ),
    ...
    it('403 in demo mode for a demo account', () => {
      ...
      try {
        expect(thrown(() => new TripsController(svc()).cover(user, '9', file))).toEqual({
          status: 403,
          body: { error: 'Uploads are disabled in demo mode. Self-host ODYSSEY for full functionality.' },
        });
      } finally {
  ...
  describe('GET /:id/export.ics', () => {
    ...
    it('404 without access, else sends the calendar with headers', () => {
      expect(
        thrown(() =>
          new TripsController(svc({ canAccessTrip: vi.fn().mockReturnValue(undefined) })).exportIcs(
            user,
            '9',
            makeRes(),
          ),
      ...
      const s = svc({
        exportICS: vi.fn().mockReturnValue({ ics: 'BEGIN:VCALENDAR', filename: 'trip.ics' }),
      } as Partial<TripsService>);
    ...
    it('maps a NotFoundError from the export to 404 and re-throws others', () => {
      ...
      expect(thrown(() => new TripsController(nf).exportIcs(user, '9', makeRes()))).toEqual({
        status: 404,
        body: { error: 'gone' },
      });
      const other = svc({
  ...
  it('POST /:id/copy maps a copy failure to 500', () => {
    ...
    expect(thrown(() => new TripsController(s).copy(user, '9', undefined, req))).toEqual({
      status: 500,
