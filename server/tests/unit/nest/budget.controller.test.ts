...
describe('BudgetController (parity with the legacy /api/trips/:tripId/budget route)', () => {
  ...
  describe('settlements ledger', () => {
    ...
    it('POST /settlements 400 when a field is missing', () => {
      const svc = makeService();
      expect(
        thrown(() => new BudgetController(svc).createSettlement(user, '5', { from_user_id: 1, to_user_id: 2 })),
      ).toEqual({
        status: 400,
        body: { error: 'from_user_id, to_user_id and amount are required' },
      });
      expect(
        thrown(() => new BudgetController(svc).createSettlement(user, '5', { from_user_id: 1, amount: 5 })),
      ).toEqual({
        status: 400,
        body: { error: 'from_user_id, to_user_id and amount are required' },
      });
    ...
    it('POST /settlements creates and broadcasts (amount 0 is allowed)', () => {
      ...
      const res = new BudgetController(svc).createSettlement(
        user,
        '5',
        { from_user_id: 1, to_user_id: 2, amount: 0 },
        'sock',
      );
    ...
    it('PUT /settlements/:id 400 when a field is missing', () => {
      const svc = makeService();
      expect(
        thrown(() => new BudgetController(svc).updateSettlement(user, '5', '7', { from_user_id: 1, to_user_id: 2 })),
      ).toEqual({
        status: 400,
        body: { error: 'from_user_id, to_user_id and amount are required' },
      });
    ...
    it('PUT /settlements/:id updates and broadcasts', () => {
      ...
      const res = new BudgetController(svc).updateSettlement(
        user,
        '5',
        '7',
  ...
  describe('POST /', () => {
    ...
    it('creates and broadcasts', () => {
      ...
      expect(new BudgetController(svc).create(user, '5', { name: 'Hotel', total_price: 200 }, 'sock')).toEqual({
  ...
  describe('PUT /:id', () => {
    ...
    it('syncs the reservation price when a linked item changes total_price', () => {
      ...
      expect(broadcast).toHaveBeenCalledWith(
        '5',
        'budget:updated',
        { item: { id: 9, reservation_id: 42, total_price: 250 } },
        'sock',
      );
    });
