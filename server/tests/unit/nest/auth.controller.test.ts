...
describe('AuthPublicController', () => {
  ...
  it('reset-password 429 once the dedicated reset bucket is exhausted', () => {
    ...
    expect(thrown(() => c.resetPassword({}, req))).toEqual({
      status: 429,
  ...
  it('demo-login + register + invite throw 429 when the login bucket is exhausted', () => {
    ...
    expect(thrown(() => c.register({}, req, res))).toEqual({
      status: 429,
      body: { error: 'Too many attempts. Please try again later.' },
    });
    expect(thrown(() => c.invite('t', req))).toEqual({
      status: 429,
      body: { error: 'Too many attempts. Please try again later.' },
    });
  });
...
describe('AuthController (authenticated)', () => {
  it('GET /me 404 when missing, else returns the loaded user', () => {
    ...
    expect(
      new AuthController(asvc({ getCurrentUser: vi.fn().mockReturnValue({ id: 1 }) } as Partial<AuthService>), rl()).me(
        user,
      ),
    ).toEqual({ user: { id: 1 } });
  });
  ...
  it('mfa/enable audits + returns backup codes; mcp-tokens create 201', () => {
    ...
    expect(enable.mfaEnable(user, { code: '123456' }, req)).toEqual({
      success: true,
      mfa_enabled: true,
      backup_codes: ['a', 'b'],
    });
  ...
  it('rate-limited account ops throw 429 once the bucket is exhausted', () => {
    ...
    expect(thrown(() => c.changePassword(user, {}, req))).toEqual({
      status: 429,
      body: { error: 'Too many attempts. Please try again later.' },
    });
  });
  ...
  it('maps-key + api-keys pass straight through to the service', () => {
    ...
    expect(
      new AuthController(asvc({ updateMapsKey } as Partial<AuthService>), rl()).mapsKey(user, { maps_api_key: 'k' }),
    ).toEqual({ success: true });
    expect(updateMapsKey).toHaveBeenCalledWith(1, 'k');
    const updateApiKeys = vi.fn().mockReturnValue({ ok: 1 });
    expect(new AuthController(asvc({ updateApiKeys } as Partial<AuthService>), rl()).apiKeys(user, { a: 1 })).toEqual({
      ok: 1,
    });
  });
  ...
  it('delete-avatar + users + travel-stats delegate to the service', async () => {
    ...
    expect(new AuthController(asvc({ listUsers } as Partial<AuthService>), rl()).users(user)).toEqual({
      users: [{ id: 1 }],
    });
    expect(listUsers).toHaveBeenCalledWith(1);
    const getTravelStats = vi.fn().mockReturnValue({ countries: 3 });
    expect(new AuthController(asvc({ getTravelStats } as Partial<AuthService>), rl()).travelStats(user)).toEqual({
      countries: 3,
    });
  });
