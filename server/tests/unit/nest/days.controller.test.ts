...
describe('DaysController (parity with the legacy /api/trips/:tripId/days route)', () => {
  it('404 when trip not accessible', () => {
    ...
    expect(thrown(() => new DaysController(svc).list(user, '5'))).toEqual({
      status: 404,
  ...
  it('POST / 403 without day_edit, then creates + broadcasts', () => {
    expect(
      thrown(() => new DaysController(daysSvc({ canEdit: vi.fn().mockReturnValue(false) })).create(user, '5', {})),
    ).toEqual({ status: 403, body: { error: 'No permission' } });
    const create = vi.fn().mockReturnValue({ id: 9 });
    const broadcast = vi.fn();
    expect(
      new DaysController(daysSvc({ create, broadcast } as Partial<DaysService>)).create(
        user,
        '5',
        { date: '2026-07-01' },
        'sock',
  ...
  it('POST / 404 when the trip is not accessible', () => {
    ...
    expect(thrown(() => new DaysController(svc).create(user, '5', {}))).toEqual({
      status: 404,
      body: { error: 'Trip not found' },
    });
  });
  ...
  it('POST / with a position inserts + broadcasts day:reordered', () => {
    const insert = vi.fn().mockReturnValue({ id: 12 });
    const create = vi.fn();
    const broadcast = vi.fn();
  ...
  describe('PUT /reorder', () => {
    it('404 when the trip is not accessible', () => {
      const svc = daysSvc({ verifyTripAccess: vi.fn().mockReturnValue(undefined) });
      expect(thrown(() => new DaysController(svc).reorder(user, '5', { orderedIds: [1, 2] }))).toEqual({
        status: 404,
        body: { error: 'Trip not found' },
      });
    });
    ...
    it('400 when orderedIds is missing', () => {
      expect(thrown(() => new DaysController(daysSvc()).reorder(user, '5', {}))).toEqual({
        status: 400,
        body: { error: 'orderedIds must be an array' },
      });
    });

    it('400 when orderedIds is not an array', () => {
      expect(thrown(() => new DaysController(daysSvc()).reorder(user, '5', { orderedIds: 'nope' as never }))).toEqual({
        status: 400,
        body: { error: 'orderedIds must be an array' },
      });
    });
  ...
  it('PUT /:id 404 when the day is missing, else updates', () => {
    expect(
      thrown(() =>
        new DaysController(daysSvc({ getDay: vi.fn().mockReturnValue(undefined) } as Partial<DaysService>)).update(
          user,
          '5',
          '9',
          {},
  ...
  it('DELETE /:id 404 when missing, else success', () => {
    expect(
      thrown(() =>
        new DaysController(daysSvc({ getDay: vi.fn().mockReturnValue(undefined) } as Partial<DaysService>)).remove(
          user,
          '5',
          '9',
        ),
...
describe('DayNotesController (parity with the legacy /api/.../days/:dayId/notes route)', () => {
  ...
  it('400 on an over-long time', () => {
    expect(
      thrown(() => new DayNotesController(notesSvc()).create(user, '5', '3', { text: 'ok', time: 'y'.repeat(151) })),
    ).toEqual({
      status: 400,
      body: { error: 'time must be 150 characters or less' },
    });
  });
  ...
  it('404 trip, 403 permission, 404 day, 400 empty text, then creates', () => {
    expect(
      thrown(() =>
        new DayNotesController(notesSvc({ verifyTripAccess: vi.fn().mockReturnValue(undefined) })).create(
          user,
          '5',
          '3',
          { text: 'ok' },
    ...
    expect(
      thrown(() =>
        new DayNotesController(notesSvc({ canEdit: vi.fn().mockReturnValue(false) })).create(user, '5', '3', {
          text: 'ok',
        }),
      ),
    ).toEqual({ status: 403, body: { error: 'No permission' } });
    expect(
      thrown(() =>
        new DayNotesController(
          notesSvc({ dayExists: vi.fn().mockReturnValue(false) } as Partial<DayNotesService>),
    ...
    expect(
      thrown(() =>
        new DayNotesController(
          notesSvc({ dayExists: vi.fn().mockReturnValue(true) } as Partial<DayNotesService>),
        ).create(user, '5', '3', { text: '  ' }),
      ),
    ).toEqual({ status: 400, body: { error: 'Text required' } });
    const create = vi.fn().mockReturnValue({ id: 7 });
    const broadcast = vi.fn();
    const svc = notesSvc({ dayExists: vi.fn().mockReturnValue(true), create, broadcast } as Partial<DayNotesService>);
    expect(new DayNotesController(svc).create(user, '5', '3', { text: 'Lunch', time: '12:00' }, 'sock')).toEqual({
      note: { id: 7 },
    });
  ...
  it('GET / returns notes; PUT/DELETE 404 when the note is missing', () => {
    ...
    expect(
      thrown(() =>
        new DayNotesController(
          notesSvc({ getNote: vi.fn().mockReturnValue(undefined) } as Partial<DayNotesService>),
  ...
  it('PUT/DELETE update + delete a note with broadcasts', () => {
    ...
    const b2 = vi.fn();
    const d = notesSvc({
      getNote: vi.fn().mockReturnValue({ id: 9 }),
      remove,
      broadcast: b2,
