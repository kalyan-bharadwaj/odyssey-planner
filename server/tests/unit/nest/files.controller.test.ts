...
describe('FilesController (parity with the legacy /api/trips/:tripId/files route)', () => {
  it('GET / 404 without access, else lists with the trash flag', () => {
    expect(
      thrown(() => new FilesController(fsvc({ verifyTripAccess: vi.fn().mockReturnValue(undefined) })).list(user, '5')),
    ).toEqual({ status: 404, body: { error: 'Trip not found' } });
    const listFiles = vi.fn().mockReturnValue([{ id: 1 }]);
    expect(new FilesController(fsvc({ listFiles } as Partial<FilesService>)).list(user, '5', 'true')).toEqual({
      files: [{ id: 1 }],
    });
  ...
  describe('POST / (upload)', () => {
    ...
    it('403 in demo mode for a demo email', () => {
      ...
      expect(thrown(() => new FilesController(fsvc()).upload(user, '5', file, {}))).toEqual({
        status: 403,
    ...
    it('403 without file_upload, 400 without a file, else creates + broadcasts', () => {
      expect(
        thrown(() => new FilesController(fsvc({ can: vi.fn().mockReturnValue(false) })).upload(user, '5', file, {})),
      ).toEqual({ status: 403, body: { error: 'No permission to upload files' } });
      expect(thrown(() => new FilesController(fsvc()).upload(user, '5', undefined, {}))).toEqual({
        status: 400,
  ...
  it('PUT /:id 403 without file_edit, 404 unknown, else updates + broadcasts', () => {
    expect(
      thrown(() => new FilesController(fsvc({ can: vi.fn().mockReturnValue(false) })).update(user, '5', '9', {})),
  ...
  it('PATCH /:id/star 403/404, else toggles', () => {
    expect(
      thrown(() => new FilesController(fsvc({ can: vi.fn().mockReturnValue(false) })).star(user, '5', '9')),
  ...
  it('DELETE /:id soft-delete 403/404, else success', () => {
    expect(
      thrown(() => new FilesController(fsvc({ can: vi.fn().mockReturnValue(false) })).remove(user, '5', '9')),
  ...
  it('POST /:id/link 404 unknown file, else links', () => {
    ...
    expect(new FilesController(s).link(user, '5', '9', { reservation_id: 2 })).toEqual({
      success: true,
      links: [{ id: 1 }],
    });
  });
  ...
  it('DELETE /:id/link/:linkId removes the link; GET /:id/links lists', () => {
    ...
    expect(new FilesController(fsvc({ deleteFileLink } as Partial<FilesService>)).unlink(user, '5', '9', '3')).toEqual({
      success: true,
    });
  ...
  it('the trash + link routes all reject without file_delete / file_edit', async () => {
    ...
    expect(thrown(() => new FilesController(denied()).restore(user, '5', '9'))).toEqual({
      status: 403,
      body: { error: 'No permission' },
    });
    expect(thrown(() => new FilesController(denied()).link(user, '5', '9', {}))).toEqual({
      status: 403,
      body: { error: 'No permission' },
    });
    expect(thrown(() => new FilesController(denied()).unlink(user, '5', '9', '3'))).toEqual({
      status: 403,
      body: { error: 'No permission' },
    });
  });
  ...
  it('GET /:id/links 404 without trip access', () => {
    ...
    expect(thrown(() => new FilesController(s).links(user, '5', '9'))).toEqual({
      status: 404,
...
describe('FilesDownloadController', () => {
  ...
  it('maps the auth error from authenticateDownload', () => {
    ...
    expect(thrown(() => new FilesDownloadController(s).download(req, res, '5', '9'))).toEqual({
      status: 401,
    ...
    expect(
      thrown(() =>
        new FilesDownloadController(
          dsvc({ resolveFilePath: vi.fn().mockReturnValue({ resolved: '/x', safe: false }) }),
  ...
  it('404 when the safe path is gone from disk', () => {
    const missing = path.join(os.tmpdir(), `odyssey-no-such-${Date.now()}.pdf`);
  ...
  it('streams a regular file via sendFile with an explicit root', () => {
    const real = path.join(os.tmpdir(), `odyssey-dl-${Date.now()}.pdf`);
  ...
  it('serves a .pkpass inline with the Wallet MIME type and the original name', () => {
    const real = path.join(os.tmpdir(), `odyssey-pass-${Date.now()}.pkpass`);
    fs.writeFileSync(real, 'x');
    try {
