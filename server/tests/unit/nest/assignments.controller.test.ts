...
function svc(o: Partial<AssignmentsService> = {}): AssignmentsService {
  return {
    ...
    placeExists: vi.fn().mockReturnValue(true),
    notifyPlaceCreated: vi.fn(),
    ...o,
...
describe('DayAssignmentsController (parity with the legacy day-assignments routes)', () => {
  it('404 trip, then 404 day on GET', () => {
    expect(
      thrown(() =>
        new DayAssignmentsController(svc({ verifyTripAccess: vi.fn().mockReturnValue(undefined) })).list(
          user,
          '5',
          '3',
        ),
    ...
    expect(
      thrown(() =>
        new DayAssignmentsController(
          svc({ dayExists: vi.fn().mockReturnValue(false) } as Partial<AssignmentsService>),
        ).list(user, '5', '3'),
      ),
  ...
  it('GET returns assignments (access-only, no permission gate)', () => {
    const s = svc({
      ...
      listDayAssignments: vi.fn().mockReturnValue([{ id: 1 }]),
  ...
  describe('POST', () => {
    it('403 without day_edit; 404 place not found; then creates + hooks', () => {
      expect(
        thrown(() =>
          new DayAssignmentsController(svc({ canEdit: vi.fn().mockReturnValue(false) })).create(user, '5', '3', {
            place_id: 2,
          }),
        ),
      ...
      expect(
        thrown(() =>
          new DayAssignmentsController(
            svc({ placeExists: vi.fn().mockReturnValue(false) } as Partial<AssignmentsService>),
          ).create(user, '5', '3', { place_id: 2 }),
        ),
      ).toEqual({ status: 404, body: { error: 'Place not found' } });
      const createAssignment = vi.fn().mockReturnValue({ id: 9 });
      const broadcast = vi.fn();
      const notifyPlaceCreated = vi.fn();
      const s = svc({ createAssignment, broadcast, notifyPlaceCreated } as Partial<AssignmentsService>);
      expect(new DayAssignmentsController(s).create(user, '5', '3', { place_id: 2, notes: 'n' }, 'sock')).toEqual({
        assignment: { id: 9 },
      });
  ...
  it('PUT /reorder 404 day, else reorders + broadcasts', () => {
    expect(
      thrown(() =>
        new DayAssignmentsController(
          svc({ dayExists: vi.fn().mockReturnValue(false) } as Partial<AssignmentsService>),
        ).reorder(user, '5', '3', [1, 2]),
      ),
    ).toEqual({ status: 404, body: { error: 'Day not found' } });
    const reorderAssignments = vi.fn();
    const broadcast = vi.fn();
    expect(
      new DayAssignmentsController(svc({ reorderAssignments, broadcast } as Partial<AssignmentsService>)).reorder(
        user,
        '5',
        '3',
        [2, 1],
  ...
  it('DELETE /:id 404 when not in day, else success', () => {
    expect(
      thrown(() =>
        new DayAssignmentsController(
          svc({ assignmentExistsInDay: vi.fn().mockReturnValue(false) } as Partial<AssignmentsService>),
        ).remove(user, '5', '3', '9'),
      ),
    ).toEqual({ status: 404, body: { error: 'Assignment not found' } });
    const s = svc({
      assignmentExistsInDay: vi.fn().mockReturnValue(true),
...
describe('AssignmentOpsController (parity with the per-assignment op routes)', () => {
  it('PUT /:id/move 404 assignment, 404 target day, else moves', () => {
    ...
    expect(
      thrown(() =>
        new AssignmentOpsController(
          svc({
            getAssignmentForTrip: vi.fn().mockReturnValue({ day_id: 3 }),
            dayExists: vi.fn().mockReturnValue(false),
          } as Partial<AssignmentsService>),
        ).move(user, '5', '9', { new_day_id: 4 }),
      ),
    ).toEqual({ status: 404, body: { error: 'Target day not found' } });
    const moveAssignment = vi.fn().mockReturnValue({ assignment: { id: 9 } });
    const broadcast = vi.fn();
    const s = svc({
      getAssignmentForTrip: vi.fn().mockReturnValue({ day_id: 3 }),
      moveAssignment,
    ...
    expect(new AssignmentOpsController(s).move(user, '5', '9', { new_day_id: 4, order_index: 0 }, 'sock')).toEqual({
      assignment: { id: 9 },
    });
    ...
    expect(broadcast).toHaveBeenCalledWith(
      '5',
      'assignment:moved',
      { assignment: { id: 9 }, oldDayId: 3, newDayId: 4 },
      'sock',
    );
  });
  ...
  it('PUT /:id/time 404 missing, else updates', () => {
    ...
    const updateTime = vi.fn().mockReturnValue({ id: 9 });
    const broadcast = vi.fn();
    const s = svc({
    ...
    expect(new AssignmentOpsController(s).time(user, '5', '9', { place_time: '10:00' }, 'sock')).toEqual({
      assignment: { id: 9 },
    });
  ...
  it('PUT /:id/participants 400 not array, else sets + broadcasts', () => {
    expect(thrown(() => new AssignmentOpsController(svc()).setParticipants(user, '5', '9', 'no'))).toEqual({
      status: 400,
      body: { error: 'user_ids must be an array' },
    });
    const setParticipants = vi.fn().mockReturnValue([{ user_id: 2 }]);
    const broadcast = vi.fn();
    expect(
      new AssignmentOpsController(svc({ setParticipants, broadcast } as Partial<AssignmentsService>)).setParticipants(
        user,
        '5',
        '9',
        [2],
    ...
    ).toEqual({ participants: [{ user_id: 2 }] });
    expect(broadcast).toHaveBeenCalledWith(
      '5',
      'assignment:participants',
      { assignmentId: 9, participants: [{ user_id: 2 }] },
      'sock',
    );
  });
});
