...
const h = vi.hoisted(() => ({
  ...
  getMcpSafeUrl: vi.fn(() => 'https://odyssey.example.test'),
  dbPrepare: vi.fn(),
  existsSync: vi.fn(),
...
vi.mock('../../../src/mcp/oauthProvider', () => ({ odysseyOAuthProvider: {}, odysseyClientsStore: {} }));
...
beforeEach(() => {
  vi.clearAllMocks();
  h.getMcpSafeUrl.mockReturnValue('https://odyssey.example.test');
});
...
describe('applyPlatformTransport', () => {
  ...
  it('GET /.well-known/openid-configuration returns AS metadata + userinfo_endpoint', () => {
    ...
    expect(body.issuer).toBe('https://odyssey.example.test');
    expect(body.userinfo_endpoint).toBe('https://odyssey.example.test/oauth/userinfo');
  });
  ...
  it('trims trailing slashes off the configured base URL', () => {
    ...
    const res = makeRes();
    handler({}, res);
    expect((res.body as { issuer: string }).issuer).toBe('https://odyssey.example.test');
  });
  ...
  describe('GET /.well-known/oauth-protected-resource (flat)', () => {
    ...
    it('returns the PRM document when MCP is enabled', () => {
      ...
      expect(body.resource).toBe('https://odyssey.example.test/mcp');
      expect(body.authorization_servers).toEqual(['https://odyssey.example.test']);
    });
  });
