...
describe('PlacesController (parity with the legacy /api/trips/:tripId/places route)', () => {
  it('GET / lists with filters; 404 when trip not accessible', () => {
    expect(
      thrown(() => new PlacesController(svc({ verifyTripAccess: vi.fn().mockReturnValue(undefined) })).list(user, '5')),
    ).toEqual({ status: 404, body: { error: 'Trip not found' } });
    const list = vi.fn().mockReturnValue([{ id: 1 }]);
    expect(
      new PlacesController(svc({ list } as Partial<PlacesService>)).list(user, '5', 'beach', 'cat', 'tag'),
    ).toEqual({ places: [{ id: 1 }] });
  ...
  describe('POST / (create)', () => {
    it('400 on an over-long name (length guard before permission)', () => {
      ...
      expect(thrown(() => new PlacesController(svc({ canEdit })).create(user, '5', { name: 'x'.repeat(201) }))).toEqual(
        {
          status: 400,
          body: { error: 'name must be 200 characters or less' },
        },
      );
    ...
    it('403 without place_edit, 400 without name, then creates + hooks', () => {
      expect(
        thrown(() =>
          new PlacesController(svc({ canEdit: vi.fn().mockReturnValue(false) })).create(user, '5', { name: 'Spot' }),
        ),
      ).toEqual({ status: 403, body: { error: 'No permission' } });
      expect(thrown(() => new PlacesController(svc()).create(user, '5', {}))).toEqual({
        status: 400,
        body: { error: 'Place name is required' },
      });
  ...
  describe('POST /import/gpx', () => {
    ...
    it('400 without a file', () => {
      expect(thrown(() => new PlacesController(svc()).importGpx(user, '5', undefined, {}))).toEqual({
        status: 400,
    ...
    it('400 when all import types are disabled', () => {
      expect(
        thrown(() =>
          new PlacesController(svc()).importGpx(user, '5', file, {
        ...
        status: 400,
        body: { error: 'No import types selected' },
      });
    });
    it('400 when the GPX yields nothing', () => {
      expect(
        thrown(() =>
          new PlacesController(svc({ importGpx: vi.fn().mockReturnValue(null) } as Partial<PlacesService>)).importGpx(
            user,
            '5',
            file,
            {},
        ...
        status: 400,
        body: { error: 'No matching places found in GPX file' },
      });
    });
    it('imports and broadcasts per place', () => {
      const broadcast = vi.fn();
      const s = svc({
        importGpx: vi.fn().mockReturnValue({ places: [{ id: 1 }, { id: 2 }], count: 2, skipped: 0 }),
        broadcast,
      } as Partial<PlacesService>);
      expect(new PlacesController(s).importGpx(user, '5', file, {}, 'sock')).toEqual({
        places: [{ id: 1 }, { id: 2 }],
        count: 2,
        skipped: 0,
      });
  ...
  describe('POST /import/map', () => {
    ...
    it('400 without a file', async () => {
      expect(await thrownAsync(() => new PlacesController(svc()).importMap(user, '5', undefined, {}))).toEqual({
        status: 400,
        body: { error: 'No file uploaded' },
      });
    });
    ...
    it('400 when both import types are disabled', async () => {
      expect(
        await thrownAsync(() =>
          new PlacesController(svc()).importMap(user, '5', file, { importPoints: 'false', importPaths: 'false' }),
        ),
      ).toEqual({
        status: 400,
        body: { error: 'No import types selected' },
    ...
    it('400 when the map file has no Placemarks (and carries the summary through)', async () => {
      ...
      expect(await thrownAsync(() => new PlacesController(s).importMap(user, '5', file, {}))).toEqual({
        status: 400,
        body: { error: 'No valid Placemarks found in map file', summary },
      });
    });
    ...
    it('falls back to a generic 400 message for a non-Error rejection', async () => {
      ...
      expect(await thrownAsync(() => new PlacesController(s).importMap(user, '5', file, {}))).toEqual({
        status: 400,
        body: { error: 'Failed to import map file' },
      });
    });
    it('re-throws an HttpException raised inside the try untouched', async () => {
      const s = svc({
        importMapFile: vi.fn().mockRejectedValue(new HttpException({ error: 'teapot' }, 418)),
      } as Partial<PlacesService>);
      expect(await thrownAsync(() => new PlacesController(s).importMap(user, '5', file, {}))).toEqual({
        status: 418,
        body: { error: 'teapot' },
  ...
  describe('POST /import/google-list + naver-list', () => {
    it('400 without a url', async () => {
      expect(await thrownAsync(() => new PlacesController(svc()).importGoogle(user, '5', undefined))).toEqual({
        status: 400,
        body: { error: 'URL is required' },
      });
    });
    it('400 when url is the wrong type (not a string)', async () => {
      expect(await thrownAsync(() => new PlacesController(svc()).importNaver(user, '5', 123))).toEqual({
        status: 400,
        body: { error: 'URL is required' },
      });
    });
    it('maps a service { error, status } to the same response', async () => {
      const s = svc({
        importGoogleList: vi.fn().mockResolvedValue({ error: 'List is empty', status: 400 }),
      } as Partial<PlacesService>);
      expect(await thrownAsync(() => new PlacesController(s).importGoogle(user, '5', 'http://x'))).toEqual({
        status: 400,
        body: { error: 'List is empty' },
      });
    });
    it('imports a naver list and returns the count + listName', async () => {
      const s = svc({
        importNaverList: vi.fn().mockResolvedValue({ places: [{ id: 1 }], listName: 'Trip', skipped: 2 }),
        broadcast: vi.fn(),
      } as Partial<PlacesService>);
      expect(await new PlacesController(s).importNaver(user, '5', 'http://x')).toEqual({
        places: [{ id: 1 }],
        count: 1,
        listName: 'Trip',
        skipped: 2,
    ...
    it('forwards the enrich flag + userId and broadcasts each imported place', async () => {
      ...
      expect(await new PlacesController(s).importGoogle(user, '5', 'http://x', 'true', 'sock')).toEqual({
        places: [{ id: 1 }, { id: 2 }],
        count: 2,
        listName: 'L',
        skipped: 0,
    ...
    it('wraps a thrown Error in the provider-specific 400 (Google)', async () => {
      const s = svc({
        importGoogleList: vi.fn().mockRejectedValue(new Error('network down')),
      } as Partial<PlacesService>);
      expect(await thrownAsync(() => new PlacesController(s).importGoogle(user, '5', 'http://x'))).toEqual({
        status: 400,
        body: { error: 'Failed to import Google Maps list. Make sure the list is shared publicly.' },
      });
    });
    it('wraps a non-Error rejection in the provider-specific 400 (Naver)', async () => {
      ...
      expect(await thrownAsync(() => new PlacesController(s).importNaver(user, '5', 'http://x'))).toEqual({
        status: 400,
        body: { error: 'Failed to import Naver Maps list. Make sure the list is shared publicly.' },
      });
    });
  });
  ...
  describe('POST /bulk-delete', () => {
    it('400 when ids is not an array of numbers', () => {
      expect(thrown(() => new PlacesController(svc()).bulkDelete(user, '5', ['a']))).toEqual({
        status: 400,
    ...
    it('returns empty for an empty list without touching the service', () => {
      ...
      expect(new PlacesController(svc({ removeMany } as Partial<PlacesService>)).bulkDelete(user, '5', [])).toEqual({
        deleted: [],
        count: 0,
      });
  ...
  it('GET /:id returns the place when found, 404 when missing', () => {
    expect(
      thrown(() =>
        new PlacesController(svc({ get: vi.fn().mockReturnValue(undefined) } as Partial<PlacesService>)).get(
          user,
          '5',
          '9',
        ),
  ...
  it('PUT /:id 404 when missing, else updates + hooks', () => {
    expect(
      thrown(() =>
        new PlacesController(svc({ update: vi.fn().mockReturnValue(null) } as Partial<PlacesService>)).update(
          user,
          '5',
          '9',
          { name: 'X' },
    ...
    expect(
      thrown(() =>
        new PlacesController(
          svc({ remove: vi.fn().mockReturnValue(false), onDeleted } as Partial<PlacesService>),
        ).remove(user, '5', '9'),
      ),
    ).toEqual({ status: 404, body: { error: 'Place not found' } });
  ...
  it('GET /:id/image maps service error + returns photos', async () => {
    ...
    const e = svc({
      searchImage: vi.fn().mockResolvedValue({ error: 'No key', status: 400 }),
    } as Partial<PlacesService>);
    expect(await thrownAsync(() => new PlacesController(e).image(user, '5', '9'))).toEqual({
      status: 400,
      body: { error: 'No key' },
    });
  });
  ...
  it('GET /:id/image turns an unexpected throw into a 500, but re-throws an HttpException as-is', async () => {
    ...
    expect(await thrownAsync(() => new PlacesController(boom).image(user, '5', '9'))).toEqual({
      status: 500,
      body: { error: 'Error searching for image' },
    });
    const http = svc({
      searchImage: vi.fn().mockRejectedValue(new HttpException({ error: 'rate limited' }, 429)),
    } as Partial<PlacesService>);
    expect(await thrownAsync(() => new PlacesController(http).image(user, '5', '9'))).toEqual({
      status: 429,
