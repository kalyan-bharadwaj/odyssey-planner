 ...
 * Also covers the resource odyssey://notifications/in-app.
 */
...
// ---------------------------------------------------------------------------
// Resource: odyssey://notifications/in-app
// ---------------------------------------------------------------------------

describe('Resource: odyssey://notifications/in-app', () => {
  it('returns notifications list', async () => {
    const { user } = createUser(testDb);
    createNotification(testDb, user.id, { title_key: 'notif.test' });
    await withResourceHarness(user.id, async (h) => {
      const result = await h.client.readResource({ uri: 'odyssey://notifications/in-app' });
