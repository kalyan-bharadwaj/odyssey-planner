...
describe('resolveOidcRole', () => {
  ...
  it('OIDC-SVC-011: returns admin when groups array contains OIDC_ADMIN_VALUE', () => {
    process.env.OIDC_ADMIN_VALUE = 'odyssey-admins';
    expect(resolveOidcRole({ sub: 'x', groups: ['odyssey-users', 'odyssey-admins'] }, false)).toBe('admin');
  });

  it('OIDC-SVC-012: returns user when groups array does not contain OIDC_ADMIN_VALUE', () => {
    process.env.OIDC_ADMIN_VALUE = 'odyssey-admins';
    expect(resolveOidcRole({ sub: 'x', groups: ['odyssey-users'] }, false)).toBe('user');
  });
...
describe('findOrCreateUser', () => {
  ...
  it('OIDC-SVC-025b: refuses to link an unverified email to an existing local account', () => {
    ...
    const result = findOrCreateUser({ sub: 'sub-dora-attacker', email: 'dora@example.com', name: 'Dora' }, MOCK_CONFIG);
...
describe('verifyIdToken', () => {
  ...
  const ISSUER = 'https://auth.example.com/application/o/odyssey';
  const CLIENT_ID = 'odyssey-client';
