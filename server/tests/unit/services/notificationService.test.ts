...
function setSmtp(): void {
  ...
  setAppSetting(testDb, 'smtp_from', 'odyssey@test.com');
}
...
function setUserNtfyTopic(userId: number, topic = 'my-odyssey-topic'): void {
  testDb
    .prepare("INSERT OR REPLACE INTO settings (user_id, key, value) VALUES (?, 'ntfy_topic', ?)")
    .run(userId, topic);
}

function setAdminNtfyTopic(topic = 'odyssey-admin-alerts'): void {
