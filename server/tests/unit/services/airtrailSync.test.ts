...
/**
 * An AirTrail flight as GET returns it, including the fields ODYSSEY doesn't model.
...
function existingFlight(
  ...
  return {
    ...
    // AirTrail-owned detail ODYSSEY never surfaces — must survive a writeback (#1240).
...
/** A linked ODYSSEY reservation (the shape getReservationWithJoins returns). */
...
describe('airtrailSync.buildSavePayload', () => {
  it('round-trips the AirTrail-owned fields ODYSSEY does not model (issue #1240)', () => {
  ...
  it('writes the ODYSSEY time to the SCHEDULED fields so it round-trips on the next pull', () => {
    // Import reads the scheduled time, so a ODYSSEY edit must be pushed back there
  ...
  it('blanks the scheduled time when the ODYSSEY reservation has only a date', () => {
  ...
  it('still applies the ODYSSEY-owned edits on top of the preserved fields', () => {
    const payload = buildSavePayload(
      reservation({
        reservation_time: '2021-09-01T20:30',
        notes: 'changed in ODYSSEY',
  ...
  it('preserves AirTrail aircraft/airline/reason when ODYSSEY metadata omits them (#1240)', () => {
    // A ODYSSEY edit can drop these AirTrail-owned fields from metadata; the writeback
