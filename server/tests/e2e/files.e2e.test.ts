...
const { photoSvc, helperSvc } = vi.hoisted(() => ({
  photoSvc: { streamPhoto: vi.fn(), getPhotoInfo: vi.fn(), resolveOdysseyPhoto: vi.fn() },
  helperSvc: { canAccessOdysseyPhoto: vi.fn() },
}));
...
describe('Files + photos e2e (real auth guard + temp SQLite)', () => {
  ...
  beforeEach(() => {
    ...
    helperSvc.canAccessOdysseyPhoto.mockReturnValue(true);
  });
  ...
  it('403 on a photo the user cannot access', async () => {
    helperSvc.canAccessOdysseyPhoto.mockReturnValue(false);
