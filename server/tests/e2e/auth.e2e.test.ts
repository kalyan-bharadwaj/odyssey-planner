 ...
 * are mocked; this proves the httpOnly odyssey_session cookie is set on login and
 * cleared on logout, that /me requires a session, and that /app-config is
 * optional-auth.
 */
import { AuthModule } from '../../src/nest/auth/auth.module';
import { OdysseyExceptionFilter } from '../../src/nest/common/odyssey-exception.filter';
...
describe('Auth e2e (real auth guard + real cookie service + temp SQLite)', () => {
  ...
  async function build() {
    ...
    nest.useGlobalFilters(new OdysseyExceptionFilter());
    await nest.init();
    return nest;
  }
  ...
  it('POST /login sets the httpOnly odyssey_session cookie', async () => {
    authSvc.loginUser.mockReturnValue({ token: 'jwt.token.value', user: { id: 1 } });
    ...
    expect(setCookie.some((c) => c.startsWith('odyssey_session=') && /HttpOnly/i.test(c))).toBe(true);
  }, 10000);
  ...
  it('POST /login with remember_me sets a persistent cookie (Max-Age present)', async () => {
    ...
    expect(res.status).toBe(200);
    const setCookie = res.headers['set-cookie'] as unknown as string[];
    const cookie = setCookie.find((c) => c.startsWith('odyssey_session='))!;
  ...
  it('POST /logout clears the session cookie', async () => {
    ...
    const setCookie = res.headers['set-cookie'] as unknown as string[];
    expect(setCookie.some((c) => c.startsWith('odyssey_session='))).toBe(true);
  });
});
