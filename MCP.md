...
ODYSSEY includes a built-in [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) server that lets AI
assistants — such as Claude Desktop, Cursor, or any MCP-compatible client — read and modify your trip data through a
structured API.

> **Note:** MCP is an addon that must be enabled by your ODYSSEY administrator before it becomes available.
...
```json
{
  "mcpServers": {
    "odyssey": {
      ...
      "args": [
        "mcp-remote",
        "https://your-odyssey-instance.com/mcp"
      ]
    }
  }
}
...
4. Your browser opens ODYSSEY's consent screen, where you choose which scopes (permissions) to grant.
...
> **Requirement:** The `APP_URL` environment variable must be set to your ODYSSEY instance's public URL for OAuth
...
(`odysseycs_` prefix) and fixed scopes that you define up front.
  ...
  "mcpServers": {
    "odyssey": {
      ...
      "args": [
        ...
        "https://your-odyssey-instance.com/mcp",
        "--header",
        "Authorization: Bearer odyssey_your_token_here"
      ]
    }
  }
}
...
ODYSSEY's MCP server supports three authentication methods. OAuth 2.1 is the recommended path for all external clients.

| Method | Token prefix | Access level | TTL | Notes |
|--------|-------------|-------------|-----|-------|
| **OAuth 2.1** | `odysseyoa_` | Scoped (per-consent) | 1 hour | Recommended. Automatically refreshed via 30-day rolling refresh tokens (`odysseyrf_` prefix). Replay-detected rotation — replayed tokens cascade-revoke the entire chain. |
| **Static API token** | `odyssey_` | Full access | No expiry | **Deprecated.** Triggers deprecation warnings in AI clients. Will be removed in a future release. |
| **Web session JWT** | — | Full access | Session-based | Used internally by the ODYSSEY web UI. Not intended for external clients. |
...
When connecting via OAuth 2.1, you grant specific scopes during the consent step. ODYSSEY registers only the MCP tools
...
| **Demo mode restrictions**              | If ODYSSEY is running in demo mode, all write operations through MCP are blocked.                                                                   |
...
Resources provide read-only access to your ODYSSEY data. MCP clients can read these to understand the current state before
making changes.
...
|-----------------------|-------------------------------------------------|---------------------------------------------------------------------------------------|
| Trips                 | `odyssey://trips`                                  | All trips you own or are a member of                                                  |
| Trip Detail           | `odyssey://trips/{tripId}`                         | Single trip with metadata and member count                                            |
| Days                  | `odyssey://trips/{tripId}/days`                    | Days of a trip with their assigned places                                             |
| Places                | `odyssey://trips/{tripId}/places`                  | All places/POIs saved in a trip. Supports `?assignment=all\|unassigned\|assigned`     |
| Budget                | `odyssey://trips/{tripId}/budget`                  | Budget and expense items                                                              |
| Budget Per-Person     | `odyssey://trips/{tripId}/budget/per-person`       | Per-person totals and split breakdown                                                 |
| Budget Settlement     | `odyssey://trips/{tripId}/budget/settlement`       | Suggested transactions to settle who owes whom                                        |
| Packing               | `odyssey://trips/{tripId}/packing`                 | Packing checklist                                                                     |
| Packing Bags          | `odyssey://trips/{tripId}/packing/bags`            | Packing bags with their assigned members                                              |
| Reservations          | `odyssey://trips/{tripId}/reservations`            | Flights, hotels, restaurants, etc.                                                    |
| Day Notes             | `odyssey://trips/{tripId}/days/{dayId}/notes`      | Notes for a specific day                                                              |
| Accommodations        | `odyssey://trips/{tripId}/accommodations`          | Hotels/rentals with check-in/out details                                              |
| Members               | `odyssey://trips/{tripId}/members`                 | Owner and collaborators                                                               |
| Collab Notes          | `odyssey://trips/{tripId}/collab-notes`            | Shared collaborative notes                                                            |
| To-Dos                | `odyssey://trips/{tripId}/todos`                   | To-do items ordered by position                                                       |
| Categories            | `odyssey://categories`                             | Available place categories (for use when creating places)                             |
| Bucket List           | `odyssey://bucket-list`                            | Your personal travel bucket list                                                      |
| Visited Countries     | `odyssey://visited-countries`                      | Countries marked as visited in Atlas                                                  |
| Notifications         | `odyssey://notifications/in-app`                   | Your in-app notifications (most recent 50, unread first)                              |
...
|-----------------------|-------------------------------------------------|----------|---------------------------------------------------------------------|
| Atlas Stats           | `odyssey://atlas/stats`                            | Atlas    | Visited country counts and continent breakdown                      |
| Atlas Regions         | `odyssey://atlas/regions`                          | Atlas    | Manually visited sub-country regions                                |
| Collab Polls          | `odyssey://trips/{tripId}/collab/polls`            | Collab   | All polls for a trip with vote counts per option                    |
| Collab Messages       | `odyssey://trips/{tripId}/collab/messages`         | Collab   | Most recent 100 chat messages for a trip                            |
| Vacay Plan            | `odyssey://vacay/plan`                             | Vacay    | Full snapshot of your active vacation plan (members, years, config) |
| Vacay Entries         | `odyssey://vacay/entries/{year}`                   | Vacay    | All vacation day entries for the active plan and a specific year    |
| Vacay Holidays        | `odyssey://vacay/holidays/{year}`                  | Vacay    | Public holidays for the plan's configured region and year           |
| Journeys              | `odyssey://journeys`                               | Journey  | All journeys owned or contributed to by the current user            |
| Journey Detail        | `odyssey://journeys/{journeyId}`                   | Journey  | Single journey with entries, contributors, and linked trips         |
| Journey Entries       | `odyssey://journeys/{journeyId}/entries`           | Journey  | All entries in a journey (date, text, mood, linked trip)            |
| Journey Contributors  | `odyssey://journeys/{journeyId}/contributors`      | Journey  | Contributors (owner and collaborators) of a journey                 |

---

## Tools (read-write)

ODYSSEY exposes tools organized by feature area. Use `get_trip_summary` as a starting point — it returns everything about a
...
| `token_auth_notice`  | Static token deprecation notice and migration guide. Only available in sessions authenticated with a legacy `odyssey_` token. |
...
Before writing anything to ODYSSEY, do some research: look up what's worth
...
Once you have a solid plan, write the whole thing to ODYSSEY:
- Create the trip
...
PDF of the generated trip: [./docs/ODYSSEY-Generated-by-MCP.pdf](./docs/ODYSSEY-Generated-by-MCP.pdf)
