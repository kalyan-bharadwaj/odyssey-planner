...
const en: NotificationLocale = {
  email: {
    footer: 'You received this because you have notifications enabled in ODYSSEY.',
    manage: 'Manage preferences in Settings',
    madeWith: 'Made with',
    openOdyssey: 'Open ODYSSEY',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} invited you to fuse vacation plans. Open ODYSSEY to accept or decline.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'New ODYSSEY version available',
      body: `ODYSSEY ${p.version} is now available. Visit the admin panel to update.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Hi',
    body: 'We received a request to reset the password for your ODYSSEY account. Click the button below to set a new password.',
