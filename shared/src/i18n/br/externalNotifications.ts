...
const br: NotificationLocale = {
  email: {
    footer: 'Você recebeu isso porque tem as notificações ativadas no ODYSSEY.',
  ...
  events: {
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} convidou você para fundir planos de férias. Abra o ODYSSEY para aceitar ou recusar.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nova versão do ODYSSEY disponível',
      body: `O ODYSSEY ${p.version} está disponível. Acesse o painel de administração para atualizar.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Olá',
    body: 'Recebemos um pedido para redefinir a senha da sua conta ODYSSEY. Clique no botão abaixo para definir uma nova senha.',
