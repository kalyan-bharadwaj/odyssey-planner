...
const id: NotificationLocale = {
  email: {
    footer: 'Anda menerima ini karena Anda telah mengaktifkan notifikasi di ODYSSEY.',
    manage: 'Kelola preferensi di Pengaturan',
    madeWith: 'Dibuat dengan',
    openOdyssey: 'Buka ODYSSEY',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} mengundang Anda untuk menggabungkan rencana liburan. Buka ODYSSEY untuk menerima atau menolak.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Versi ODYSSEY baru tersedia',
      body: `ODYSSEY ${p.version} sekarang tersedia. Kunjungi panel admin untuk memperbarui.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Halo',
    body: 'Kami menerima permintaan untuk menyetel ulang kata sandi akun ODYSSEY Anda. Klik tombol di bawah untuk menetapkan kata sandi baru.',
