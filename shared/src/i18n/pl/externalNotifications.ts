...
const pl: NotificationLocale = {
  email: {
    footer: 'Otrzymałeś/aś tę wiadomość, ponieważ masz włączone powiadomienia w ODYSSEY.',
    manage: 'Zarządzaj preferencjami w ustawieniach',
    madeWith: 'Made with',
    openOdyssey: 'Otwórz ODYSSEY',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} zaprosił Cię do połączenia planów urlopowych. Otwórz ODYSSEY, aby zaakceptować lub odrzucić.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nowa wersja ODYSSEY dostępna',
      body: `ODYSSEY ${p.version} jest teraz dostępny. Odwiedź panel administracyjny, aby zaktualizować.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Cześć',
    body: 'Otrzymaliśmy prośbę o zresetowanie hasła do Twojego konta ODYSSEY. Kliknij przycisk poniżej, aby ustawić nowe hasło.',
