...
const admin: TranslationStrings = {
  ...
  'admin.addons.subtitle': '機能を有効／無効にしてODYSSEYをカスタマイズします。',
  ...
  'admin.weather.description':
    'ODYSSEYは天気データにOpen‑Meteoを使用しています。無料でオープンソース、APIキーは不要です。',
  ...
  'admin.github.by': '作成者',
  'admin.github.support': 'ODYSSEYの開発を支援',
  'admin.update.available': '更新があります',
  'admin.update.text': 'ODYSSEY {version} が利用可能です。現在は {current} を使用しています。',
  'admin.update.button': 'GitHubで見る',
  'admin.update.install': '更新をインストール',
  'admin.update.confirmTitle': '更新をインストールしますか？',
  'admin.update.confirmText': 'ODYSSEYを {current} から {version} に更新します。更新後、サーバーは自動的に再起動します。',
  ...
  'admin.update.dockerText':
    'ODYSSEYはDockerで実行されています。{version} に更新するには、サーバーで次のコマンドを実行してください：',
  ...
  'admin.passkey.rpIdHint':
    'パスキーが紐づくドメイン名のみ（例：odyssey.example.org）。空欄の場合は APP_URL から導出されます。後で変更すると既存のパスキーは無効になります。',
  'admin.passkey.origins': '許可するオリジン',
  'admin.passkey.originsHint':
    'カンマ区切りの完全なオリジン（例：https://odyssey.example.org）。空欄の場合は APP_URL を使用します。',
