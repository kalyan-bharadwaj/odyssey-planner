...
const ja: NotificationLocale = {
  email: {
    footer: 'ODYSSEYで通知を有効にしているため、このメールが届きました。',
    manage: '設定で通知設定を管理',
    madeWith: 'Made with',
    openOdyssey: 'ODYSSEYを開く',
  },
  events: {
    trip_invite: (p) => ({
      title: `「${p.trip}」への旅行招待`,
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor}が休暇プランの統合に招待しています。ODYSSEYを開いて承認または拒否してください。`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: '新しいODYSSEYバージョンが利用可能',
      body: `ODYSSEY ${p.version}が利用可能になりました。管理パネルからアップデートしてください。`,
    }),
  ...
  passwordReset: {
    subject: 'パスワードをリセット',
    greeting: 'こんにちは',
    body: 'ODYSSEYアカウントのパスワードリセットリクエストを受け付けました。以下のボタンをクリックして新しいパスワードを設定してください。',
    ctaIntro: 'パスワードをリセット',
