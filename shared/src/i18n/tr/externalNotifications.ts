...
const tr: NotificationLocale = {
  email: {
    footer: "ODYSSEY'te bildirimleri etkinleştirdiğiniz için bunu aldınız.",
    manage: 'Ayarlarda tercihleri yönetin',
    madeWith: 'Made with',
    openOdyssey: "ODYSSEY'i aç",
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} sizi tatil planlarını birleştirmeye davet etti. Kabul etmek veya reddetmek için ODYSSEY'i açın.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Yeni ODYSSEY sürümü mevcut',
      body: `ODYSSEY ${p.version} artık mevcut. Güncellemek için yönetici panelini ziyaret edin.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Merhaba',
    body: 'ODYSSEY hesabınızın şifresini sıfırlamak için bir istek aldık. Yeni bir şifre belirlemek için aşağıdaki butona tıklayın.',
