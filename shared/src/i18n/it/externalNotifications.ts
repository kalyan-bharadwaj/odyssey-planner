...
const it: NotificationLocale = {
  email: {
    footer: 'Hai ricevuto questa email perché hai le notifiche abilitate in ODYSSEY.',
    manage: 'Gestisci le preferenze nelle impostazioni',
    madeWith: 'Made with',
    openOdyssey: 'Apri ODYSSEY',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} ti ha invitato a fondere i piani vacanza. Apri ODYSSEY per accettare o rifiutare.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nuova versione ODYSSEY disponibile',
      body: `ODYSSEY ${p.version} è ora disponibile. Visita il pannello di amministrazione per aggiornare.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Ciao',
    body: 'Abbiamo ricevuto una richiesta di reimpostazione della password per il tuo account ODYSSEY. Clicca il pulsante qui sotto per impostare una nuova password.',
