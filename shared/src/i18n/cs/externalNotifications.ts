...
const cs: NotificationLocale = {
  email: {
    footer: 'Toto jsi obdržel/a, protože máš povoleny upozornění v ODYSSEY.',
    manage: 'Spravovat předvolby v nastavení',
    madeWith: 'Made with',
    openOdyssey: 'Otevřít ODYSSEY',
  },
  events: {
    trip_invite: (p) => ({
    ...
    vacay_invite: (p) => ({
      ...
      body: `${p.actor} vás pozval ke spojení dovolenkových plánů. Otevřete ODYSSEY pro přijetí nebo odmítnutí.`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: 'Nová verze ODYSSEY dostupná',
      body: `ODYSSEY ${p.version} je nyní dostupný. Navštivte administrátorský panel pro aktualizaci.`,
    }),
  ...
  passwordReset: {
    ...
    greeting: 'Ahoj',
    body: 'Obdrželi jsme žádost o obnovení hesla k tvému účtu ODYSSEY. Klikni na tlačítko níže a nastav nové heslo.',
