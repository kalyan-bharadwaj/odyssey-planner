...
const zh: NotificationLocale = {
  email: {
    footer: '您收到此邮件是因为您在 ODYSSEY 中启用了通知。',
    manage: '管理偏好设置',
    madeWith: 'Made with',
    openOdyssey: '打开 ODYSSEY',
  },
  events: {
    trip_invite: (p) => ({
      title: `邀请加入"${p.trip}"`,
    ...
    vacay_invite: (p) => ({
      title: 'Vacay 融合邀请',
      body: `${p.actor} 邀请你合并假期计划。打开 ODYSSEY 接受或拒绝。`,
    }),
    photos_shared: (p) => ({
    ...
    version_available: (p) => ({
      title: '新版 ODYSSEY 可用',
      body: `ODYSSEY ${p.version} 现已可用。请前往管理面板进行更新。`,
    }),
  ...
  passwordReset: {
    subject: '重置您的密码',
    greeting: '您好',
    body: '我们收到了重置您的 ODYSSEY 账户密码的请求。点击下方按钮设置新密码。',
    ctaIntro: '重置密码',
